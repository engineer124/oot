...
s32 Camera_Normal1(Camera* camera) {
    ...
    if (norm1->interfaceFlags & NORM1_FLG_1) {
    ...
    if (norm1->interfaceFlags & NORM1_FLG_80) {
        func_800458D4(camera, &atEyeNextGeo, spA0, &anim->yOffset, norm1->interfaceFlags & NORM1_FLG_1);
    } else if (norm1->interfaceFlags & NORM1_FLG_20) {
        func_80045B08(camera, &atEyeNextGeo, spA0, anim->slopePitchAdj);
    } else {
        Camera_CalcAtDefault(camera, &atEyeNextGeo, spA0, norm1->interfaceFlags & NORM1_FLG_1);
    }
    ...
    if ((camera->status == CAM_STAT_ACTIVE) && (!(norm1->interfaceFlags & NORM1_FLG_10))) {
        ...
        if (norm1->interfaceFlags & NORM1_FLG_4) {
...
s32 Camera_Normal2(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 0x19:
            ...
            if (norm2->interfaceFlags & NORM2_FLG_4) {
    ...
    if (!(norm2->interfaceFlags & NORM2_FLG_80)) {
        Camera_CalcAtDefault(camera, &atToEyeNextDir, norm2->unk_00, norm2->interfaceFlags & NORM2_FLG_1);
    } else {
        func_800458D4(camera, &atToEyeNextDir, norm2->unk_00, &anim->unk_24, norm2->interfaceFlags & NORM2_FLG_1);
    }
    ...
    if (camera->status == CAM_STAT_ACTIVE) {
        bgChk.pos = *eyeNext;
        if (!camera->globalCtx->envCtx.skyboxDisabled || norm2->interfaceFlags & NORM2_FLG_10) {
...
s32 Camera_Parallel1(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 0x19:
            anim->unk_16 = 0;
            anim->unk_10 = 0;
            if (para1->interfaceFlags & PARA1_FLG_4) {
                anim->animTimer = 20;
            } else {
    ...
    if (anim->animTimer != 0) {
        ...
        if (para1->interfaceFlags & PARA1_FLG_20) {
            anim->yawTarget = BINANG_ROT180(playerPosRot->rot.y) + para1->yawTarget;
        }
    ...
    if (para1->interfaceFlags & PARA1_FLG_1) {
    ...
    if (anim->animTimer != 0) {
        camera->engineFlags |= CAM_ENGINE_FLG_20;
        ...
        if (para1->interfaceFlags & PARA1_FLG_40) {
    ...
    if (camera->status == CAM_STAT_ACTIVE) {
        sp6C.pos = *eyeNext;
        if (!camera->globalCtx->envCtx.skyboxDisabled || para1->interfaceFlags & PARA1_FLG_10) {
...
s32 Camera_Parallel3(Camera* camera) {
    ...
    if (val & PARA3_FLG_1) {
        camera->engineFlags |= CAM_ENGINE_FLG_400;
    }
    if (val & PARA3_FLG_2) {
        camera->engineFlags |= CAM_ENGINE_FLG_10;
    }
    //! @bug doesn't return
}
...
s32 Camera_Jump1(Camera* camera) {
    ...
    if ((camera->status == CAM_STAT_ACTIVE) && !(jump1->interfaceFlags & JUMP1_FLG_10)) {
        func_80046E20(camera, &eyeDiffSph, jump1->distMin, jump1->yawUpateRateTarget, &spA4, &anim->swing);
        if (jump1->interfaceFlags & JUMP1_FLG_4) {
...
s32 Camera_Jump2(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        if (jump2->interfaceFlags & JUMP2_FLG_2) {
            anim->yawAdj = 0xA;
        } else {
            anim->yawAdj = 0x2710;
        }
...
s32 Camera_Jump3(Camera* camera) {
    ...
    if ((camera->status == CAM_STAT_ACTIVE) && !(jump3->interfaceFlags & JUMP3_FLG_10)) {
        func_80046E20(camera, &eyeDiffSph, jump3->distMin, jump3->swingUpdateRate, &spBC, &anim->swing);
        if (jump3->interfaceFlags & JUMP3_FLG_4) {
            camera->inputDir.x = -eyeAtOffset.pitch;
            camera->inputDir.y = BINANG_ROT180(eyeAtOffset.yaw);
            camera->inputDir.z = 0;
        } else {
...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (!skipEyeAtCalc) {
        ...
        spBC.pos = *eyeNext;
        if (camera->status == CAM_STAT_ACTIVE) {
            if (!camera->globalCtx->envCtx.skyboxDisabled || batt1->flags & BATT1_FLG_1) {
                Camera_BGCheckInfo(camera, at, &spBC);
            } else if (batt1->flags & BATT1_FLG_2) {
...
s32 Camera_KeepOn1(Camera* camera) {
    ...
    switch (camera->paramFlags & (CAM_PARAMS_FLG_10 | CAM_PARAMS_FLG_8)) {
        case CAM_PARAMS_FLG_8:
    ...
    if (sp88 == 0) {
        ...
        sp8C.pos = *eyeNext;
        if (camera->status == CAM_STAT_ACTIVE) {
            if (!camera->globalCtx->envCtx.skyboxDisabled || keep1->interfaceFlags & KEEP1_FLG_1) {
                Camera_BGCheckInfo(camera, at, &sp8C);
            } else if (keep1->interfaceFlags & KEEP1_FLG_2) {
...
s32 Camera_KeepOn3(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        ...
        camera->engineFlags &= ~(CAM_ENGINE_FLG_8 | CAM_ENGINE_FLG_4);
    ...
    if (anim->animTimer != 0) {
        ...
        *eye = *eyeNext;
        ...
        camera->engineFlags |= (CAM_ENGINE_FLG_400 | CAM_ENGINE_FLG_10);
    }

    if (camera->engineFlags & CAM_ENGINE_FLG_8) {
        sCameraInterfaceFlags = CAM_SHRINKWINVAL_NONE | CAM_IFACE_ALPHA(0);
        ...
        if (camera->xzSpeed > 0.001f || CHECK_BTN_ALL(D_8015BD7C->state.input[0].press.button, BTN_A) ||
            ...
            camera->engineFlags |= CAM_ENGINE_FLG_4;
            camera->engineFlags &= ~CAM_ENGINE_FLG_8;
        }
    }
    return 1;
}
...
s32 Camera_KeepOn4(Camera* camera) {
    ...
    if (camera->animState == 0 || camera->animState == 0xA || camera->animState == 0x14) {
        if (camera->globalCtx->view.unk_124 == 0) {
            camera->engineFlags |= CAM_ENGINE_FLG_20;
            camera->engineFlags &= ~(CAM_ENGINE_FLG_4 | CAM_ENGINE_FLG_2);
            camera->globalCtx->view.unk_124 = camera->thisIdx | 0x50;
            return 1;
        }
    ...
    if (RELOAD_PARAMS) {
        ...
        switch (*temp_s0) {
            ...
            case 9:
                ...
                keep4->unk_08 = -20.0f;
                keep4->unk_0C = 0.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(5) | KEEP4_FLG_40;
                break;
            case 5:
                keep4->unk_00 = playerHeight * -0.4f * yNormal;
                keep4->unk_08 = -10.0f;
                keep4->unk_0C = 45.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(0) | KEEP4_FLG_2;
                break;
            case 10:
                ...
                keep4->unk_0C = 175.0f;
                keep4->unk_18 = 70.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(2) | KEEP4_FLG_2;
                keep4->unk_1E = 0x3C;
                break;
            case 12:
                keep4->unk_00 = playerHeight * -0.6f * yNormal;
                ...
                keep4->interfaceFlags = CAM_SHRINKWINVAL_LARGE | CAM_IFACE_ALPHA(2) | KEEP4_FLG_10 | KEEP4_FLG_2;
                keep4->unk_1E = 0x1E;
                keep4->unk_18 = 50.0f;
                break;
            case 0x5A:
                ...
                keep4->unk_18 = 45.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(0xF) | KEEP4_FLG_2;
                break;
            case 0x5B:
                keep4->unk_00 = playerHeight * -0.1f * yNormal;
                keep4->unk_04 = playerHeight * 1.5f * yNormal;
                ...
                keep4->unk_0C = 10.0f;
                keep4->unk_18 = 55.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(0xF) | KEEP4_FLG_8;
                break;
            case 0x51:
                keep4->unk_00 = playerHeight * -0.3f * yNormal;
                keep4->unk_04 = playerHeight * 1.5f * yNormal;
                ...
                keep4->unk_0C = 20.0f;
                keep4->unk_10 = 20.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(2) | KEEP4_FLG_80;
                keep4->unk_1E = 0x1E;
                keep4->unk_18 = 45.0f;
                break;
            case 11:
                ...
                keep4->unk_0C = 130.0f;
                keep4->unk_10 = 10.0f;
                keep4->interfaceFlags = CAM_SHRINKWINVAL_MEDIUM | CAM_IFACE_ALPHA(5) | KEEP4_FLG_20 | KEEP4_FLG_2;
                break;
            default:
                break;
        }
    ...
    if (unk20->unk_10 != 0) {
        ...
        unk20->unk_10--;
    } else if (keep4->interfaceFlags & KEEP4_FLG_10) {
        camera->engineFlags |= (CAM_ENGINE_FLG_400 | CAM_ENGINE_FLG_10);
        camera->engineFlags |= (CAM_ENGINE_FLG_4 | CAM_ENGINE_FLG_2);
        camera->engineFlags &= ~CAM_ENGINE_FLG_8;
        if (camera->timer > 0) {
        ...
        camera->engineFlags |= (CAM_ENGINE_FLG_400 | CAM_ENGINE_FLG_10);
        if ((camera->engineFlags & CAM_ENGINE_FLG_8) || (keep4->interfaceFlags & KEEP4_FLG_80)) {
            sCameraInterfaceFlags = CAM_SHRINKWINVAL_NONE | CAM_IFACE_ALPHA(0);
            camera->engineFlags |= (CAM_ENGINE_FLG_4 | CAM_ENGINE_FLG_2);
            camera->engineFlags &= ~CAM_ENGINE_FLG_8;
...
s32 Camera_Fixed2(Camera* camera) {
    ...
    if (camera->animState == 0) {
        camera->animState++;
        func_80043B60(camera);
        if (!(fixd2->interfaceFlags & FIXD2_FLG_1)) {
...
s32 Camera_Fixed4(Camera* camera) {
    ...
    if (camera->animState == 0) {
        camera->animState++;
        if (!(fixed4->interfaceFlags & FIXD4_FLG_4)) {
            func_80043B60(camera);
        }
...
s32 Camera_Unique2(Camera* camera) {
    ...
    if (camera->animState == 0) {
        ...
        func_80043B60(camera);
        unk10->unk_00 = 200.0f;
        if (uniq2->interfaceFlags & UNIQ2_FLG_10) {
            camera->engineFlags &= ~CAM_ENGINE_FLG_4;
        }
    }

    playerPos = camera->playerPosRot.pos;
    lerpRateFactor = (uniq2->interfaceFlags & UNIQ2_FLG_1) ? 1.0f : camera->speedRatio;
    ...
    if (uniq2->interfaceFlags & UNIQ2_FLG_1) {
    ...
    } else if (uniq2->interfaceFlags & UNIQ2_FLG_2) {
...
s32 Camera_Unique3(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 2:
            if (params->interfaceFlags & UNIQ3_FLG_4) {
        ...
        case 4:
            if (params->interfaceFlags & UNIQ3_FLG_2) {
                camera->engineFlags |= CAM_ENGINE_FLG_4;
                camera->engineFlags &= ~CAM_ENGINE_FLG_8;
...
s32 Camera_Unique0(Camera* camera) {
    ...
    if (params->interfaceFlags & UNIQ0_FLG_1) {
        if (anim->animTimer > 0) {
            anim->animTimer--;
            anim->initalPos = playerPosRot->pos;
...
s32 Camera_Unique9(Camera* camera) {
    ...
    switch (action) {
        ...
        case 21:
            ...
            if (anim->unk_38 == 0) {
                anim->unk_38 = 1;
            } else if (camera->engineFlags & CAM_ENGINE_FLG_8) {
                anim->unk_38 = 0;
                camera->engineFlags &= ~CAM_ENGINE_FLG_8;
            }
            *at = anim->atTarget;
            *eyeNext = anim->eyeTarget;
            ...
            break;
        case 16:
            // same as 16, but don't unset bit CAM_ENGINE_FLG_8 on engineFlags
            if (anim->unk_38 == 0) {
                anim->unk_38 = 1;
            } else if (camera->engineFlags & CAM_ENGINE_FLG_8) {
                anim->unk_38 = 0;
...
s32 Camera_Demo3(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        default:
            camera->engineFlags |= (CAM_ENGINE_FLG_10 | CAM_ENGINE_FLG_4);
            camera->engineFlags &= ~CAM_ENGINE_FLG_8;
            if (camera->prevCamDataIdx < 0) {
                Camera_ChangeSettingFlags(camera, camera->prevSetting, 2);
            } else {
...
s32 Camera_Demo5(Camera* camera) {
    ...
    Actor_GetFocus(&playerhead, &camera->player->actor);
    player = camera->player;
    sCameraInterfaceFlags = CAM_SHRINKWINVAL_LARGE | CAM_IFACE_ALPHA(2);
    if ((camera->target == NULL) || (camera->target->update == NULL)) {
        if (camera->target == NULL) {
...
/**
 * No combination of setting and mode uses Camera_Demo7
 */
s32 Camera_Demo7(Camera* camera) {
    if (camera->animState == 0) {
        camera->engineFlags &= ~CAM_ENGINE_FLG_4;
        camera->engineFlags |= CAM_ENGINE_FLG_1000;
        camera->animState++;
    }
    //! @bug doesn't return
}
    ...
    switch (camera->animState) {
        ...
        case 1:
            ...
            if (spec9->doorParams.timer1 <= 0) {
                camera->animState++;
                if (params->interfaceFlags & SPEC9_FLG_1) {
                    camPosData = Camera_GetCamBGData(camera);
...
void Camera_Init(Camera* camera, View* view, CollisionContext* colCtx, GlobalContext* globalCtx) {
    ...
    camera->csId = 0x7FFF;
    camera->timer = -1;
    camera->engineFlags |= CAM_ENGINE_FLG_4000;
...
s32 Camera_CheckWater(Camera* camera) {
    ...
    if (!(camera->engineFlags & CAM_ENGINE_FLG_2) || sCameraSettings[camera->setting].unk_00 & 0x40000000) {
        return 0;
    }

    if (camera->engineFlags & CAM_ENGINE_FLG_200) {
        if (player->stateFlags2 & 0x800) {
            Camera_ChangeSettingFlags(camera, CAM_SET_CIRCLE5, 6);
            camera->engineFlags |= CAM_ENGINE_FLG_8000;
        } else if (camera->engineFlags & CAM_ENGINE_FLG_8000) {
            Camera_ChangeSettingFlags(camera, *waterPrevCamSetting, 6);
            camera->engineFlags &= ~CAM_ENGINE_FLG_8000;
        }
    }
    if (!(camera->engineFlags & CAM_ENGINE_FLG_8000)) {
        if (waterCamIdx = Camera_GetWaterBoxDataIdx(camera, &waterY), waterCamIdx == -2) {
            // No camera data idx
            if (!(camera->engineFlags & CAM_ENGINE_FLG_200)) {
                camera->engineFlags |= CAM_ENGINE_FLG_200;
        ...
        } else if (camera->engineFlags & CAM_ENGINE_FLG_200) {
    ...
    if (waterY = Camera_GetWaterSurface(camera, &camera->eye, &waterLightsIndex), waterY != BGCHECK_Y_MIN) {
        camera->waterYPos = waterY;
        if (!(camera->engineFlags & CAM_ENGINE_FLG_100)) {
            camera->engineFlags |= CAM_ENGINE_FLG_100;
        ...
        if (camera->unk_150 > 0) {
            camera->unk_150--;
            camera->viewFlags |= CAM_VIEW_FLG_8;
        } else if (camera->globalCtx->sceneNum == SCENE_TURIBORI) {
            camera->viewFlags |= CAM_VIEW_FLG_10;
        } else {
            camera->viewFlags |= CAM_VIEW_FLG_2;
        }
    } else {
        if (camera->engineFlags & CAM_ENGINE_FLG_100) {
            camera->engineFlags &= ~CAM_ENGINE_FLG_100;
...
s32 Camera_SetRoomHotFlag(Camera* camera) {
    camera->viewFlags &= ~CAM_VIEW_FLG_1;
...
void func_80058E8C(Camera* camera) {
    ...
    if (camera->viewFlags != 0) {
        if (camera->viewFlags & CAM_VIEW_FLG_4) {
            phi_f0 = 0.0f;
            phi_f2 = 170.0f;
            sp3C = 0.01f;
            sp40 = -0.01f;
            ...
            sp60 = 1.0f;
        } else if (camera->viewFlags & CAM_VIEW_FLG_8) {
            phi_f0 = 248.0f;
            phi_f2 = -90.0f;
            sp38 = 0.2f;
            sp34 = 0.2f;
            ...
            sp60 = 1.0f;
        } else if (camera->viewFlags & CAM_VIEW_FLG_2) {
            phi_f0 = 359.2f;
            phi_f2 = -18.5f;
            sp40 = 0.09f;
            sp38 = 0.01f;
            ...
            sp60 = phi_f20;
        } else if (camera->viewFlags & CAM_VIEW_FLG_1) {
            // hot room flag
            phi_f2 = 150.0f;
            phi_f0 = 0.0f;
            sp3C = 0.01f;
        ...
        // Without a return, these functions do nothing
        ...
        camera->engineFlags |= CAM_ENGINE_FLG_40;
    } else if (camera->engineFlags & CAM_ENGINE_FLG_40) {
...
Vec3s Camera_Update(Camera* camera) {
    ...
    if (camera->player != NULL) {
        ...
        if (sOOBTimer < 200) {
            ...
            if (!(camera->engineFlags & CAM_ENGINE_FLG_4)) {
                camera->nextCamDataIdx = -1;
            }

            if ((camera->engineFlags & CAM_ENGINE_FLG_1) && (camera->engineFlags & CAM_ENGINE_FLG_4) &&
                (!(camera->engineFlags & CAM_ENGINE_FLG_400)) &&
                (!(camera->engineFlags & CAM_ENGINE_FLG_200) || (player->currentBoots == PLAYER_BOOTS_IRON)) &&
                (!(camera->engineFlags & CAM_ENGINE_FLG_8000)) && (playerGroundY != BGCHECK_Y_MIN)) {
    ...
    camera->infoFlags = 0;
    camera->engineFlags &= ~(CAM_ENGINE_FLG_400 | CAM_ENGINE_FLG_20);
    camera->engineFlags |= CAM_ENGINE_FLG_10;

    if (R_DBG_CAM_UPDATE) {
        osSyncPrintf("camera: engine (%d %d %d) %04x \n", camera->setting, camera->mode,
                     sCameraSettings[camera->setting].cameraModes[camera->mode].funcIdx, camera->engineFlags);
    }
    ...
    if (camera->paramFlags & CAM_PARAMS_FLG_4) {
        camera->paramFlags &= ~CAM_PARAMS_FLG_4;
        viewUp = camera->up;
    } else {
        camera->up = viewUp;
    }
...
void Camera_Finish(Camera* camera) {
    ...
    if (camera->timer == 0) {
        ...
        if ((camera->parentCamIdx == MAIN_CAM) && (camera->csId != 0)) {
            ...
            mainCam->engineFlags |= CAM_ENGINE_FLG_8;
        }
...
s32 func_8005A02C(Camera* camera) {
    camera->engineFlags |= (CAM_ENGINE_FLG_8 | CAM_ENGINE_FLG_4);
    camera->engineFlags &= ~(CAM_ENGINE_FLG_1000 | CAM_ENGINE_FLG_8);
    return true;
}
...
s32 Camera_ChangeModeFlags(Camera* camera, s16 mode, u8 flags) {
    ...
    if ((camera->engineFlags & CAM_ENGINE_FLG_20) && (flags == 0)) {
        camera->infoFlags |= CAM_INFO_FLG_20;
        return -1;
    }
    ...
    if (!((sCameraSettings[camera->setting].unk_00 & 0x3FFFFFFF) & (1 << mode))) {
        ...
        if (camera->mode != CAM_MODE_NORMAL) {
            ...
            camera->infoFlags |= CAM_INFO_FLG_20;
            camera->infoFlags |= CAM_INFO_FLG_2;
            return 0;
        }
    } else {
...
s16 Camera_ChangeSettingFlags(Camera* camera, s16 setting, s16 flags) {
    ...
    if ((setting == camera->setting) && (!(flags & 1))) {
        camera->infoFlags |= CAM_INFO_FLG_10;
        if (!(flags & 2)) {
            camera->infoFlags |= CAM_INFO_FLG_1;
        }
        return -1;
    }
...
s32 Camera_ChangeDataIdx(Camera* camera, s32 camDataIdx) {
    ...
    if (camDataIdx == -1 || camDataIdx == camera->camDataIdx) {
        camera->infoFlags |= CAM_INFO_FLG_40;
        return -1;
    }

    if (!(camera->infoFlags & CAM_INFO_FLG_40)) {
        ...
        if (settingChangeSuccessful || sCameraSettings[camera->setting].unk_00 & 0x80000000) {
            ...
            camera->infoFlags |= CAM_INFO_FLG_4;
            Camera_CopyModeValuesToPREG(camera, camera->mode);
...
s32 Camera_SetParam(Camera* camera, s32 paramFlag, void* value) {
    ...
    if (value != NULL) {
        switch (paramFlag) {
            case CAM_PARAMS_FLG_1:
                camera->paramFlags &= ~(CAM_PARAMS_FLG_10 | CAM_PARAMS_FLG_8 | CAM_PARAMS_FLG_1);
...
s32 Camera_UnsetParam(Camera* camera, s16 paramFlag) {
    camera->paramFlags &= ~paramFlag;
    return true;
}

s32 Camera_OverwriteEngine(Camera* camera, s16 flags) {
    camera->engineFlags = flags;
    return true;
}
...
s16 Camera_SetEngine(Camera* camera, s16 flags) {
    camera->engineFlags |= flags;
    return camera->engineFlags;
}

s16 Camera_UnsetEngine(Camera* camera, s16 flags) {
    camera->engineFlags &= ~flags;
    return camera->engineFlags;
}
...
s16 func_8005B1A4(Camera* camera) {
    ...
    if ((camera->thisIdx == MAIN_CAM) && (camera->globalCtx->activeCamera != MAIN_CAM)) {
        GET_ACTIVE_CAM(camera->globalCtx)->engineFlags |= CAM_ENGINE_FLG_8;
