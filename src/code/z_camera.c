...
s32 Camera_CalcAtDefault(Camera* camera, VecGeo* eyeAtDir, f32 extraYOffset, s16 calcSlope) {
    ...
    Camera_SmoothStepToCeilVec3f(&posOffsetTarget, &camera->posOffset, camera->yOffsetUpdateRate,
    ...
    Camera_SmoothStepToCeilVec3f(&atTarget, at, camera->atLERPStepScale, camera->atLERPStepScale, 0.2f);
...
s32 func_800458D4(Camera* camera, VecGeo* eyeAtDir, f32 arg2, f32* arg3, s16 arg4) {
    ...
    Camera_SmoothStepToCeilVec3f(&posOffsetTarget, &camera->posOffset, CAM_DATA_SCALED(OREG(29)),
    ...
    Camera_SmoothStepToCeilVec3f(&atTarget, &camera->at, camera->atLERPStepScale, camera->atLERPStepScale, 0.2f);
...
s32 Camera_CalcAtForParallel(Camera* camera, VecGeo* arg1, f32 yOffset, f32* arg3, s16 arg4) {
    ...
    if (camera->playerGroundY == camera->playerPosRot.pos.y || camera->player->actor.gravity > -0.1f ||
        ...
        *arg3 = Camera_SmoothStepToCeilF(playerPosRot->pos.y, *arg3, CAM_DATA_SCALED(OREG(43)), 0.1f);
...
s32 Camera_CalcAtForLockOn(Camera* camera, VecGeo* eyeAtDir, Vec3f* targetPos, f32 yOffset, f32 distance,
    ...
    if (camera->playerGroundY == camera->playerPosRot.pos.y || camera->player->actor.gravity > -0.1f ||
        camera->player->stateFlags1 & PLAYER_STATE1_21) {
        *yPosOffset = Camera_SmoothStepToCeilF(playerPosRot->pos.y, *yPosOffset, CAM_DATA_SCALED(OREG(43)), 0.1f);
        yPosDelta = playerPosRot->pos.y - *yPosOffset;
        tmpPos0.y -= yPosDelta;
        Camera_SmoothStepToCeilVec3f(&tmpPos0, &camera->posOffset, camera->yOffsetUpdateRate,
                                     camera->xzOffsetUpdateRate, 0.1f);
    } else {
        if (!(flags & FLG_OFFGROUND)) {
            yPosDelta = playerPosRot->pos.y - *yPosOffset;
        ...
        Camera_SmoothStepToCeilVec3f(&tmpPos0, &camera->posOffset, CAM_DATA_SCALED(OREG(29)), CAM_DATA_SCALED(OREG(30)),
                                     0.1f);
        camera->yOffsetUpdateRate = CAM_DATA_SCALED(OREG(29));
        camera->xzOffsetUpdateRate = CAM_DATA_SCALED(OREG(30));
    }
    ...
    Camera_SmoothStepToCeilVec3f(&tmpPos1, at, camera->atLERPStepScale, camera->atLERPStepScale, 0.2f);
    return 1;
}
...
s32 Camera_CalcAtForHorse(Camera* camera, VecGeo* eyeAtDir, f32 yOffset, f32* yPosOffset, s16 calcSlope) {
    ...
    if (EN_HORSE_CHECK_JUMPING((EnHorse*)player->rideActor)) {
        ...
        *yPosOffset = Camera_SmoothStepToCeilF(horsePosRot.pos.y, *yPosOffset, 0.1f, 0.2f);
        camera->atLERPStepScale = Camera_SmoothStepToCeilF(0.4f, camera->atLERPStepScale, 0.2f, 0.02f);
    } else {
        *yPosOffset = Camera_SmoothStepToCeilF(horsePosRot.pos.y, *yPosOffset, 0.5f, 0.2f);
    }
...
f32 Camera_LERPClampDist(Camera* camera, f32 dist, f32 min, f32 max) {
    ...
    camera->rUpdateRateInv = Camera_SmoothStepToCeilF(rUpdateRateInvTarget, camera->rUpdateRateInv,
                                                      CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
    return Camera_SmoothStepToCeilF(distTarget, camera->dist, 1.0f / camera->rUpdateRateInv, 0.2f);
}
...
s16 Camera_CalcDefaultPitch(Camera* camera, s16 arg1, s16 arg2, s16 arg3) {
    ...
    return Camera_SmoothStepToCeilS(target, arg1, stepScale, 0xA);
}
...
s32 Camera_Normal1(Camera* camera) {
    ...
    if (rwData->swing.swingUpdateRateTimer != 0) {
        camera->yawUpdateRateInv =
            Camera_SmoothStepToCeilF(rwData->swing.swingUpdateRate + (f32)(rwData->swing.swingUpdateRateTimer * 2),
                                     camera->yawUpdateRateInv, sp98, rate);
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF((f32)R_CAM_PITCH_UPDATE_RATE_INV + (f32)(rwData->swing.swingUpdateRateTimer * 2),
                                     camera->pitchUpdateRateInv, sp9C, rate);
        rwData->swing.swingUpdateRateTimer--;
    } else {
        camera->yawUpdateRateInv = Camera_SmoothStepToCeilF(
            rwData->swing.swingUpdateRate - ((OREG(49) * 0.01f) * rwData->swing.swingUpdateRate * sp94),
            camera->yawUpdateRateInv, sp98, rate);
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF(R_CAM_PITCH_UPDATE_RATE_INV, camera->pitchUpdateRateInv, sp9C, rate);
    }

    camera->pitchUpdateRateInv =
        Camera_SmoothStepToCeilF(R_CAM_PITCH_UPDATE_RATE_INV, camera->pitchUpdateRateInv, sp9C, rate);
    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate, spA0, rate);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, sp9C, rate);
    camera->fovUpdateRate = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->yOffsetUpdateRate,
    ...
    if (roData->interfaceField & NORMAL1_FLAG_0) {
        ...
        rwData->slopePitchAdj =
            Camera_SmoothStepToCeilS(t, rwData->slopePitchAdj, ((1.0f / roData->unk_10) * 0.5f) + sp9C, 0xF);
    } else {
        rwData->slopePitchAdj = 0;
    ...
    if (rwData->startSwingTimer <= 0) {
        ...
        eyeAdjustment.yaw =
            Camera_SmoothStepToCeilS(rwData->swingYawTarget, atEyeNextGeo.yaw, 1.0f / camera->yawUpdateRateInv, 0xA);
    } else if (rwData->swing.unk_18 != 0) {
        eyeAdjustment.yaw =
            Camera_SmoothStepToCeilS(rwData->swing.unk_16, atEyeNextGeo.yaw, 1.0f / camera->yawUpdateRateInv, 0xA);
        eyeAdjustment.pitch =
            Camera_SmoothStepToCeilS(rwData->swing.unk_14, atEyeNextGeo.pitch, 1.0f / camera->yawUpdateRateInv, 0xA);
    } else {
    ...
    if ((camera->status == CAM_STAT_ACTIVE) && !(roData->interfaceField & NORMAL1_FLAG_4)) {
        ...
        if (rwData->swing.unk_18 != 0) {
            camera->inputDir.y = Camera_SmoothStepToCeilS(
    ...
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget * spA0, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5f, 0xA);
...
s32 Camera_Normal2(Camera* camera) {
    ...
    camera->yawUpdateRateInv = Camera_SmoothStepToCeilF(roData->unk_0C, camera->yawUpdateRateInv * camera->speedRatio,
                                                        CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
    camera->pitchUpdateRateInv =
        Camera_SmoothStepToCeilF(R_CAM_PITCH_UPDATE_RATE_INV, camera->pitchUpdateRateInv, spA0, 0.1f);
    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate, spA4, 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, spA0, 0.1f);
    camera->fovUpdateRate = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->yOffsetUpdateRate,
    ...
    if ((phi_a1 < 0x4000 && ABS(phi_a0) > phi_a1) || (phi_a1 >= 0x4000 && ABS(phi_a0) < phi_a1)) {
        ...
        phi_a0 += sp88.yaw;
        adjGeo.yaw = Camera_SmoothStepToCeilS(phi_a0, atToEyeDir.yaw,
    ...
    camera->fov = Camera_SmoothStepToCeilF(rwData->unk_1C, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5f, 0xA);
...
s32 Camera_Normal3(Camera* camera) {
    ...
    if (rwData->swing.swingUpdateRateTimer != 0) {
        ...
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF((f32)R_CAM_PITCH_UPDATE_RATE_INV + (rwData->swing.swingUpdateRateTimer * 2),
                                     camera->pitchUpdateRateInv, sp94, 0.1f);
        if (1) {}
        rwData->swing.swingUpdateRateTimer--;
    } else {
        camera->yawUpdateRateInv =
            Camera_SmoothStepToCeilF(roData->yawUpdateSpeed, camera->yawUpdateRateInv, sp98, 0.1f);
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF(R_CAM_PITCH_UPDATE_RATE_INV, camera->pitchUpdateRateInv, sp94, 0.1f);
    }

    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate, sp98, 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, sp94, 0.1f);
    camera->fovUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->fovUpdateRate, sp94, 0.1f);
    ...
    rwData->curPitch = Camera_SmoothStepToCeilS(t2, rwData->curPitch, sp94 + temp_f0, 0xF);
    ...
    sp84.pitch = Camera_SmoothStepToCeilS(phi_a0, sp74.pitch, 1.0f / camera->pitchUpdateRateInv, 0xA);
    ...
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5f, 0xA);
...
s32 Camera_Parallel1(Camera* camera) {
    ...
    camera->rUpdateRateInv = Camera_SmoothStepToCeilF(R_CAM_R_UPDATE_RATE_INV, camera->rUpdateRateInv, spB8, 0.1f);
    camera->yawUpdateRateInv = Camera_SmoothStepToCeilF(roData->unk_08, camera->yawUpdateRateInv, spB8, 0.1f);
    camera->pitchUpdateRateInv = Camera_SmoothStepToCeilF(2.0f, camera->pitchUpdateRateInv, spB4, 0.1f);
    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate, spB8, 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, spB4, 0.1f);
    camera->fovUpdateRate = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->fovUpdateRate,
                                                     camera->speedRatio * 0.05f, 0.1f);
    ...
    if (roData->interfaceField & PARALLEL1_FLAG_0) {
        ...
        rwData->unk_10 = Camera_SmoothStepToCeilS(tangle, rwData->unk_10, spB8 + pad2, 0xF);
    } else {
        rwData->unk_10 = 0;
    }
    ...
    if (rwData->animTimer != 0) {
        ...
        rwData->unk_16 = 0;
        camera->dist = Camera_SmoothStepToCeilF(roData->distTarget, camera->dist, 1.0f / camera->rUpdateRateInv, 2.0f);
        OLib_Vec3fDiffToVecGeo(&spA8, at, eyeNext);
        spA8.r = camera->dist;

        if (roData->interfaceField & PARALLEL1_FLAG_6) {
            spA8.yaw = Camera_SmoothStepToCeilS(rwData->yawTarget, atToEyeNextDir.yaw, 0.6f, 0xA);
        } else {
            spA8.yaw = Camera_SmoothStepToCeilS(rwData->yawTarget, atToEyeNextDir.yaw, 0.8f, 0xA);
        }
        ...
        spA8.pitch = Camera_SmoothStepToCeilS(phi_a0, atToEyeNextDir.pitch, 1.0f / camera->pitchUpdateRateInv, 4);
    ...
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5, 0xA);
...
s32 Camera_Jump1(Camera* camera) {
    ...
    if (rwData->swing.swingUpdateRateTimer != 0) {
        camera->yawUpdateRateInv =
            Camera_SmoothStepToCeilF(roData->yawUpateRateTarget + rwData->swing.swingUpdateRateTimer,
                                     camera->yawUpdateRateInv, CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF((f32)R_CAM_PITCH_UPDATE_RATE_INV + rwData->swing.swingUpdateRateTimer,
                                     camera->pitchUpdateRateInv, CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
        rwData->swing.swingUpdateRateTimer--;
    } else {
        camera->yawUpdateRateInv = Camera_SmoothStepToCeilF(roData->yawUpateRateTarget, camera->yawUpdateRateInv,
                                                            CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
        camera->pitchUpdateRateInv =
            Camera_SmoothStepToCeilF((f32)R_CAM_PITCH_UPDATE_RATE_INV, camera->pitchUpdateRateInv,
                                     CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
    }

    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate,
                                 CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate,
                                 CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
    camera->fovUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->yOffsetUpdateRate, 0.05f, 0.1f);
    ...
    eyeDiffGeo.r = Camera_SmoothStepToCeilF(eyeDiffTarget.r, eyeAtOffset.r, CAM_DATA_SCALED(OREG(29)), 1.0f);
    eyeDiffGeo.pitch = Camera_SmoothStepToCeilS(eyeDiffTarget.pitch, eyeAtOffset.pitch, CAM_DATA_SCALED(OREG(29)), 0xA);

    if (rwData->swing.unk_18) {
        eyeDiffGeo.yaw =
            Camera_SmoothStepToCeilS(rwData->swing.unk_16, eyeNextAtOffset.yaw, 1.0f / camera->yawUpdateRateInv, 0xA);
        eyeDiffGeo.pitch =
            Camera_SmoothStepToCeilS(rwData->swing.unk_14, eyeNextAtOffset.pitch, 1.0f / camera->yawUpdateRateInv, 0xA);
    } else {
        eyeDiffGeo.yaw =
...
s32 Camera_Jump2(Camera* camera) {
    ...
    camera->yawUpdateRateInv = Camera_SmoothStepToCeilF(roData->yawUpdRateTarget, camera->yawUpdateRateInv, sp90, 0.1f);
    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(roData->xzUpdRateTarget, camera->xzOffsetUpdateRate, sp90, 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, sp8C, 0.1f);
    ...
    if (rwData->animTimer != 0) {
        ...
        rwData->animTimer--;
        adjAtToEyeDir.yaw = Camera_SmoothStepToCeilS(rwData->yawTarget, atToEyeNextDir.yaw, 0.5f, 0xA);
    } else if (rwData->yawAdj < ABS(yawDiff)) {
        playerYawRot180 = playerPosRot->rot.y - 0x7FFF;
        adjAtToEyeDir.yaw = Camera_SmoothStepToFloorS(
            ((yawDiff < 0) ? (s16)(playerYawRot180 + rwData->yawAdj) : (s16)(playerYawRot180 - rwData->yawAdj)),
            atToEyeNextDir.yaw, 0.1f, 0xA);
    } else {
        adjAtToEyeDir.yaw = Camera_SmoothStepToCeilS(adjAtToEyeDir.yaw, atToEyeNextDir.yaw, 0.25f, 0xA);
    }
    ...
    if ((sp90 != BGCHECK_Y_MIN) && (playerPosRot->pos.y < sp90)) {
        ...
        camera->pitchUpdateRateInv = Camera_SmoothStepToCeilF(20.0f, camera->pitchUpdateRateInv,
                                                              CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
        camera->rUpdateRateInv = Camera_SmoothStepToCeilF(20.0f, camera->rUpdateRateInv,
                                                          CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y), 0.1f);
        adjAtToEyeDir.pitch =
            Camera_SmoothStepToCeilS(0x1F4, atToEyeNextDir.pitch, 1.0f / camera->pitchUpdateRateInv, 0xA);
    } else if ((playerPosRot->pos.y - rwData->floorY) < playerHeight) {
    ...
    if (Camera_BGCheckInfo(camera, at, &camBgChk)) {
        ...
        if (Camera_BGCheckInfo(camera, at, &camBgChk)) {
            ...
            *eye = bgChkPos;
            ...
            adjAtToEyeDir.pitch = Camera_SmoothStepToCeilS(0, adjAtToEyeDir.pitch, 0.2f, 0xA);
...
s32 Camera_Jump3(Camera* camera) {
    ...
    if (rwData->swing.swingUpdateRateTimer != 0) {
        camera->yawUpdateRateInv =
            Camera_SmoothStepToCeilF(rwData->swing.swingUpdateRate + (rwData->swing.swingUpdateRateTimer * 2),
                                     camera->yawUpdateRateInv, spC4, 0.1f);
        camera->pitchUpdateRateInv = Camera_SmoothStepToCeilF((rwData->swing.swingUpdateRateTimer * 2) + 40.0f,
                                                              camera->pitchUpdateRateInv, spC0, 0.1f);
        rwData->swing.swingUpdateRateTimer--;
    } else {
        camera->yawUpdateRateInv =
            Camera_SmoothStepToCeilF(rwData->swing.swingUpdateRate, camera->yawUpdateRateInv, spBC, 0.1f);
        camera->pitchUpdateRateInv = Camera_SmoothStepToCeilF(40.0f, camera->pitchUpdateRateInv, spC0, 0.1f);
    }

    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_XZ_OFFSET_UPDATE_RATE), camera->xzOffsetUpdateRate, spC4, 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate, spC0, 0.1f);
    camera->fovUpdateRate = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE), camera->yOffsetUpdateRate,
                                                     camera->speedRatio * 0.05f, 0.1f);
...
s32 Camera_Battle1(Camera* camera) {
    ...
    if (camera->player->stateFlags1 & PLAYER_STATE1_12) {
        // charging sword.
        rwData->unk_10 = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(OREG(12)) * 0.5f, rwData->unk_10,
                                                  CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
        camera->xzOffsetUpdateRate = Camera_SmoothStepToCeilF(0.2f, camera->xzOffsetUpdateRate,
                                                              CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
        camera->yOffsetUpdateRate = Camera_SmoothStepToCeilF(0.2f, camera->yOffsetUpdateRate,
                                                             CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
        ...
        rwData->chargeTimer = 40;
        rwData->unk_10 = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(OREG(12)), rwData->unk_10,
                                                  CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
        camera->xzOffsetUpdateRate = Camera_SmoothStepToCeilF(
        ...
        camera->yOffsetUpdateRate = Camera_SmoothStepToCeilF(
    ...
    spB4.r = camera->dist = Camera_SmoothStepToCeilF(distance, camera->dist, CAM_DATA_SCALED(OREG(11)), 2.0f);
    ...
    if (rwData->animTimer != 0) {
        if (rwData->animTimer >= OREG(24)) {
            ...
            spB4.r = Camera_SmoothStepToCeilF(playerToTargetDir.r + (var3 * sp86), atToEyeDir.r,
                                              CAM_DATA_SCALED(OREG(28)), 1.0f);
            spB4.yaw = Camera_SmoothStepToCeilS(playerToTargetDir.yaw + (tmpAng1 * sp86), atToEyeDir.yaw,
                                                CAM_DATA_SCALED(OREG(28)), 0xA);
            spB4.pitch = Camera_SmoothStepToCeilS(playerToTargetDir.pitch + (tmpAng2 * sp86), atToEyeDir.pitch,
                                                  CAM_DATA_SCALED(OREG(28)), 0xA);
        } else {
            skipEyeAtCalc = true;
        }
    ...
    if (!skipEyeAtCalc) {
        ...
        spB4.pitch = Camera_SmoothStepToCeilS(tmpAng1, atToEyeNextDir.pitch, rwData->unk_10, 0xA);
    ...
    camera->fov = Camera_SmoothStepToCeilF((player->meleeWeaponState != 0 ? 0.8f
...
s32 Camera_Battle4(Camera* camera) {
    ...
    camera->yawUpdateRateInv =
        Camera_SmoothStepToCeilF(roData->lerpUpdateRate, camera->yawUpdateRateInv,
                                 CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ) * camera->speedRatio, 0.1f);
    camera->rUpdateRateInv = 1000.0f;
    camera->pitchUpdateRateInv = 1000.0f;
    camera->xzOffsetUpdateRate = Camera_SmoothStepToCeilF(0.025f, camera->xzOffsetUpdateRate,
                                                          CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_Y_OFFSET_UPDATE_RATE), camera->yOffsetUpdateRate,
                                 CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y) * camera->speedRatio, 0.1f);
    ...
    if (rwData->animTimer != 0) {
        ...
        eyeNextOffset.yaw = eyeAtOffset.yaw;
        eyeNextOffset.pitch =
            Camera_SmoothStepToCeilS(roData->pitchTarget, eyeAtOffset.pitch, roData->lerpUpdateRate, 2);
        eyeNextOffset.r = Camera_SmoothStepToCeilF(roData->rTarget, eyeAtOffset.r, roData->lerpUpdateRate, 0.001f);
    }
    Camera_AddVecGeoToVec3f(eyeNext, at, &eyeNextOffset);
    *eye = *eyeNext;
    camera->dist = eyeNextOffset.r;
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, roData->lerpUpdateRate, 1.0f);
    camera->roll = 0;
...
s32 Camera_KeepOn1(Camera* camera) {
    ...
    switch (camera->viewFlags & (CAM_VIEW_TARGET | CAM_VIEW_TARGET_POS)) {
        case CAM_VIEW_TARGET:
            ...
            camera->xzOffsetUpdateRate =
                Camera_SmoothStepToCeilF(1.0f, camera->xzOffsetUpdateRate,
                                         CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ) * camera->speedRatio, 0.1f);
            camera->yOffsetUpdateRate =
                Camera_SmoothStepToCeilF(1.0f, camera->yOffsetUpdateRate,
                                         CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_Y) * camera->speedRatio, 0.1f);
            camera->fovUpdateRate = Camera_SmoothStepToCeilF(CAM_DATA_SCALED(R_CAM_FOV_UPDATE_RATE),
    ...
    camera->rUpdateRateInv =
        Camera_SmoothStepToCeilF(spE8, camera->rUpdateRateInv, CAM_DATA_SCALED(R_CAM_UPDATE_RATE_STEP_SCALE_XZ), 0.1f);
    spD8.r = spE8 = camera->dist = Camera_SmoothStepToCeilF(sp104, camera->dist, 1.0f / camera->rUpdateRateInv, 0.2f);
    ...
    spF0 = OREG(13) + spEC;
    spD8.r = camera->dist = Camera_SmoothStepToCeilF(spE8, camera->dist, CAM_DATA_SCALED(OREG(11)), 2.0f);
    spD8.yaw = spB8.yaw;
    ...
    if (rwData->unk_16 != 0) {
        if (rwData->unk_16 >= OREG(24)) {
            ...
            spD8.r = Camera_SmoothStepToCeilF(spC8.r + (spE8 * sp82), spC0.r, CAM_DATA_SCALED(OREG(28)), 1.0f);
            spD8.yaw = Camera_SmoothStepToCeilS(spC8.yaw + (spE2 * sp82), spC0.yaw, CAM_DATA_SCALED(OREG(28)), 0xA);
            spD8.pitch =
                Camera_SmoothStepToCeilS(spC8.pitch + (spE0 * sp82), spC0.pitch, CAM_DATA_SCALED(OREG(28)), 0xA);
        } else {
            sp88 = 1;
        }
        rwData->unk_16--;
    ...
    if (sp88 == 0) {
        ...
        spD8.pitch = Camera_SmoothStepToCeilS(spE2, spB8.pitch, CAM_DATA_SCALED(OREG(12)), 0xA);
    ...
    camera->fov = Camera_SmoothStepToCeilF(roData->unk_20, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5f, 0xA);
...
s32 Camera_KeepOn3(Camera* camera) {
    ...
    if (rwData->animTimer != 0) {
        ...
        *eye = *eyeNext;
        camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, 0.5, 1.0f);
        camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5, 0xA);
        camera->atLERPStepScale = Camera_ClampLERPScale(camera, roData->atLERPScaleMax);
        Camera_BGCheck(camera, at, eye);
        rwData->animTimer--;
...
s32 Camera_KeepOn4(Camera* camera) {
    ...
    Camera_SmoothStepToCeilVec3f(&D_8015BD50, at, 0.5f, 0.5f, 0.2f);
    ...
    camera->atLERPStepScale = 0.0f;
    camera->dist = Camera_SmoothStepToCeilF(roData->unk_04, camera->dist, 0.25f, 2.0f);
    spB8.r = camera->dist;
    if (rwData->unk_10 != 0) {
        camera->stateFlags |= CAM_STATE_5;
    ...
    spB8.yaw = Camera_SmoothStepToCeilS(rwData->unk_0C, spA8.yaw, roData->unk_14, 4);
    spB8.pitch = Camera_SmoothStepToCeilS(rwData->unk_0E, spA8.pitch, roData->unk_14, 4);
    Camera_AddVecGeoToVec3f(eyeNext, at, &spB8);
    *eye = *eyeNext;
    Camera_BGCheck(camera, at, eye);
    camera->fov = Camera_SmoothStepToCeilF(roData->unk_18, camera->fov, camera->fovUpdateRate, 1.0f);
    camera->roll = Camera_SmoothStepToCeilS(0, camera->roll, 0.5f, 0xA);
}
...
s32 Camera_KeepOn0(Camera* camera) {
    ...
    *eye = *eyeNext;
    ...
    camera->fov = Camera_SmoothStepToCeilF(rwData->fovTarget, camera->fov, 0.5f, 10.0f);
    return true;
}
...
s32 Camera_Fixed1(Camera* camera) {
    ...
    Camera_SmoothStepToCeilVec3f(&rwData->eyePosRotTarget.pos, eye, 0.1f, 0.1f, 0.2f);
    adjustedPos = playerPosRot->pos;
    adjustedPos.y += playerHeight;
    camera->dist = OLib_Vec3fDist(&adjustedPos, eye);

    eyeOffset.r = camera->dist;
    eyeOffset.pitch = Camera_SmoothStepToCeilS(-rwData->eyePosRotTarget.rot.x, eyeAtOffset.pitch, roData->lerpStep, 5);
    eyeOffset.yaw = Camera_SmoothStepToCeilS(rwData->eyePosRotTarget.rot.y, eyeAtOffset.yaw, roData->lerpStep, 5);

    Camera_AddVecGeoToVec3f(at, eye, &eyeOffset);

    camera->eyeNext = *eye;

    camera->fov = Camera_SmoothStepToCeilF(roData->fov, camera->fov, roData->lerpStep, 0.01f);
    camera->roll = 0;
    camera->atLERPStepScale = 0.0f;
...
s32 Camera_Fixed2(Camera* camera) {
    ...
    Camera_SmoothStepToCeilVec3f(&posOffsetTarget, &camera->posOffset, roData->posStepScale, roData->posStepScale,
                                 0.1f);
    atTarget.x = playerPosRot->pos.x + camera->posOffset.x;
    atTarget.y = playerPosRot->pos.y + camera->posOffset.y;
    atTarget.z = playerPosRot->pos.z + camera->posOffset.z;
    ...
    Camera_SmoothStepToCeilVec3f(&atTarget, &camera->at, roData->posStepScale, roData->posStepScale, 10.0f);
    Camera_SmoothStepToCeilVec3f(&rwData->eye, eyeNext, roData->eyeStepScale, roData->eyeStepScale, 0.1f);
...
s32 Camera_Fixed4(Camera* camera) {
    ...
    *eye = *eyeNext;
    ...
    posOffsetTarget.z = 0.0f;
    Camera_SmoothStepToCeilVec3f(&posOffsetTarget, &camera->posOffset, 0.1f, 0.1f, 0.1f);
    ...
    atEyeNextOffset.pitch = Camera_SmoothStepToCeilS(atTargetEyeNextOffset.pitch, atEyeNextOffset.pitch,
                                                     rwData->followSpeed * camera->speedRatio, 0xA);
    atEyeNextOffset.yaw = Camera_SmoothStepToCeilS(atTargetEyeNextOffset.yaw, atEyeNextOffset.yaw,
                                                   rwData->followSpeed * camera->speedRatio, 0xA);
...
s32 Camera_Subj3(Camera* camera) {
    ...
    if (rwData->animTimer != 0) {
        ...
        sp7C.r = Camera_SmoothStepToCeilF(sp84.r + (sp58 * rwData->animTimer), sp7C.r, CAM_DATA_SCALED(OREG(28)), 1.0f);
        sp7C.yaw =
            Camera_SmoothStepToCeilS(sp84.yaw + (sp52 * rwData->animTimer), sp7C.yaw, CAM_DATA_SCALED(OREG(28)), 0xA);
        sp7C.pitch = Camera_SmoothStepToCeilS(sp84.pitch + (sp50 * rwData->animTimer), sp7C.pitch,
                                              CAM_DATA_SCALED(OREG(28)), 0xA);
    ...
    camera->posOffset.y = camera->at.y - playerPosRot->pos.y;
    camera->posOffset.z = camera->at.z - playerPosRot->pos.z;
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, 0.25f, 1.0f);
    camera->roll = 0;
    camera->atLERPStepScale = 0.0f;
    return 1;
}
...
s32 Camera_Unique1(Camera* camera) {
    ...
    camera->yawUpdateRateInv =
        Camera_SmoothStepToCeilF(100.0f, camera->yawUpdateRateInv, R_CAM_UPDATE_RATE_STEP_SCALE_XZ * 0.01f, 0.1f);
    camera->pitchUpdateRateInv =
        Camera_SmoothStepToCeilF(100.0f, camera->pitchUpdateRateInv, R_CAM_UPDATE_RATE_STEP_SCALE_XZ * 0.01f, 0.1f);
    camera->xzOffsetUpdateRate =
        Camera_SmoothStepToCeilF(0.005f, camera->xzOffsetUpdateRate, R_CAM_UPDATE_RATE_STEP_SCALE_XZ * 0.01f, 0.01f);
    camera->yOffsetUpdateRate =
        Camera_SmoothStepToCeilF(0.01f, camera->yOffsetUpdateRate, R_CAM_UPDATE_RATE_STEP_SCALE_Y * 0.01f, 0.01f);
    camera->fovUpdateRate = Camera_SmoothStepToCeilF(R_CAM_FOV_UPDATE_RATE * 0.01f, camera->fovUpdateRate, 0.05f, 0.1f);
    ...
    sp8C.pitch = Camera_SmoothStepToCeilS(phiTarget, eyeNextAtOffset.pitch, 1.0f / camera->pitchUpdateRateInv, 0xA);
    ...
    sp8C.yaw = Camera_SmoothStepToFloorS(rwData->yawTarget, eyeNextAtOffset.yaw, 0.5f, 0x2710);
...
s32 Camera_Unique2(Camera* camera) {
    ...
    if (roData->interfaceField & UNIQUE2_FLAG_0) {
        ...
        Camera_SmoothStepToCeilVec3f(&playerPos, eye, 0.25f, 0.25f, 0.2f);
    } else if (roData->interfaceField & UNIQUE2_FLAG_1) {
        if (OLib_Vec3fDistXZ(at, eyeNext) < roData->distTarget) {
            OLib_Vec3fDiffToVecGeo(&eyeOffset, at, eyeNext);
            eyeOffset.yaw = Camera_SmoothStepToCeilS(eyeOffset.yaw, eyeAtOffset.yaw, 0.1f, 0xA);
            eyeOffset.r = roData->distTarget;
            eyeOffset.pitch = 0;
            Camera_AddVecGeoToVec3f(eye, at, &eyeOffset);
            eye->y = eyeNext->y;
        } else {
            Camera_SmoothStepToCeilVec3f(eyeNext, eye, 0.25f, 0.25f, 0.2f);
        }
    }
    ...
    camera->dist = OLib_Vec3fDist(at, eye);
    camera->roll = 0;
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov, 0.2f, 0.1f);
    camera->atLERPStepScale = Camera_ClampLERPScale(camera, 1.0f);
    return true;
}
...
s32 Camera_Unique3(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 4:
            ...
            FALLTHROUGH;
        case 5:
            camera->fov = Camera_SmoothStepToCeilF(rwData->initialFov, camera->fov, 0.4f, 0.1f);
            OLib_Vec3fDiffToVecGeo(&sp60, at, &camera->eye);
            sp60.r = Camera_SmoothStepToCeilF(100.0f, sp60.r, 0.4f, 0.1f);
...
s32 Camera_Unique7(Camera* camera) {
    ...
    *eye = *eyeNext;
    ...
    // 0x7D0 ~ 10.98 degres.
    rwData->unk_00.x = Camera_SmoothStepToFloorS(playerPosEyeOffset.yaw, rwData->unk_00.x, 0.4f, 0x7D0);
...
s32 Camera_Unique9(Camera* camera) {
    ...
    switch (action) {
        ...
        setEyeNext:
            camera->eyeNext.x =
                Camera_SmoothStepToFloorF(eyeTarget.x, camera->eyeNext.x, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->eyeNext.y =
                Camera_SmoothStepToFloorF(eyeTarget.y, camera->eyeNext.y, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->eyeNext.z =
                Camera_SmoothStepToFloorF(eyeTarget.z, camera->eyeNext.z, rwData->curKeyFrame->lerpStepScale, 1.0f);
            FALLTHROUGH;
        case 9:
        case 10:
            ...
            camera->at.x =
                Camera_SmoothStepToFloorF(atTarget.x, camera->at.x, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->at.y =
                Camera_SmoothStepToFloorF(atTarget.y, camera->at.y, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->at.z =
                Camera_SmoothStepToFloorF(atTarget.z, camera->at.z, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->fov = Camera_SmoothStepToFloorF(F32_LERPIMP(camera->fov, rwData->fovTarget, invKeyFrameTimer),
                                                    camera->fov, rwData->curKeyFrame->lerpStepScale, 0.01f);
            camera->roll =
                Camera_SmoothStepToFloorS(BINANG_LERPIMPINV(camera->roll, rwData->rollTarget, rwData->keyFrameTimer),
        ...
        case 4:
            ...
            OLib_Vec3fDiffToVecGeo(&eyeNextAtOffset, at, eyeNext);
            OLib_Vec3fDiffToVecGeo(&rwData->atEyeOffsetTarget, &rwData->atTarget, &rwData->eyeTarget);
            scratchGeo.r = Camera_SmoothStepToCeilF(rwData->atEyeOffsetTarget.r, eyeNextAtOffset.r,
                                                    rwData->curKeyFrame->lerpStepScale, 0.1f);
            scratchGeo.pitch = Camera_SmoothStepToCeilS(rwData->atEyeOffsetTarget.pitch, eyeNextAtOffset.pitch,
                                                        rwData->curKeyFrame->lerpStepScale, 1);
            scratchGeo.yaw = Camera_SmoothStepToCeilS(rwData->atEyeOffsetTarget.yaw, eyeNextAtOffset.yaw,
                                                      rwData->curKeyFrame->lerpStepScale, 1);
            Camera_AddVecGeoToVec3f(eyeNext, at, &scratchGeo);
            goto setAtFOVRoll;
        case 3:
            // linear interplation of eye/at/fov/roll using the step scale using eyeTarget
            camera->eyeNext.x = Camera_SmoothStepToCeilF(rwData->eyeTarget.x, camera->eyeNext.x,
                                                         rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->eyeNext.y = Camera_SmoothStepToCeilF(rwData->eyeTarget.y, camera->eyeNext.y,
                                                         rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->eyeNext.z = Camera_SmoothStepToCeilF(rwData->eyeTarget.z, camera->eyeNext.z,
                                                         rwData->curKeyFrame->lerpStepScale, 1.0f);
            FALLTHROUGH;
        case 11:
        case 12:
        setAtFOVRoll:
            ...
            camera->at.x =
                Camera_SmoothStepToCeilF(rwData->atTarget.x, camera->at.x, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->at.y =
                Camera_SmoothStepToCeilF(rwData->atTarget.y, camera->at.y, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->at.z =
                Camera_SmoothStepToCeilF(rwData->atTarget.z, camera->at.z, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->fov =
                Camera_SmoothStepToCeilF(rwData->fovTarget, camera->fov, rwData->curKeyFrame->lerpStepScale, 1.0f);
            camera->roll =
                Camera_SmoothStepToCeilS(rwData->rollTarget, camera->roll, rwData->curKeyFrame->lerpStepScale, 1);
            break;
        case 13:
            ...
            scratchGeo.r = !(spB4 < rwData->curKeyFrame->eyeTargetInit.z)
                               ? Camera_SmoothStepToCeilF(rwData->curKeyFrame->eyeTargetInit.z, spB4,
                                                          rwData->curKeyFrame->lerpStepScale, 1.0f)
                               : scratchGeo.r;

            Camera_AddVecGeoToVec3f(eyeNext, at, &scratchGeo);
            camera->fov = Camera_SmoothStepToCeilF(
...
s32 Camera_Demo3(Camera* camera) {
    ...
    if (!skipUpdateEye) {
        eyeOffset.r = Camera_SmoothStepToCeilF(eyeOffset.r, eyeAtOffset.r, rwData->unk_0C, 2.0f);
        eyeOffset.pitch = Camera_SmoothStepToCeilS(eyeOffset.pitch, eyeAtOffset.pitch, rwData->unk_0C, 0xA);
        eyeOffset.yaw = Camera_SmoothStepToCeilS(eyeOffset.yaw, eyeAtOffset.yaw, rwData->unk_0C, 0xA);
...
s32 Camera_Demo6(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 1:
            ...
            FALLTHROUGH;
        case 2:
            Camera_SmoothStepToCeilVec3f(&rwData->atTarget, &camera->at, 0.1f, 0.1f, 8.0f);
            if (stateTimers[camera->animState] < rwData->animTimer) {
                camera->animState++;
            } else {
                break;
            ...
            FALLTHROUGH;
        case 3:
            camera->fov = Camera_SmoothStepToCeilF(50.0f, camera->fov, 0.2f, 0.01f);
            if (stateTimers[camera->animState] < rwData->animTimer) {
                camera->timer = 0;
                return true;
            }
    ...
    switch (camera->animState) {
        ...
        case 1:
            ...
            if (rwData->animTimer > 0) {
                ...
                *eye = *eyeNext;
                if (rwData->doLERPAt) {
                    Camera_SmoothStepToCeilVec3f(&newAt, at, 0.5f, 0.5f, 0.1f);
                } else {
                    *at = newAt;
...
s32 Camera_Special0(Camera* camera) {
    ...
    Actor_GetFocus(&camera->targetPosRot, camera->target);
    Camera_SmoothStepToCeilVec3f(&camera->targetPosRot.pos, &camera->at, roData->lerpAtScale, roData->lerpAtScale,
                                 0.1f);
...
s32 Camera_Special4(Camera* camera) {
    ...
    camera->fov = Camera_SmoothStepToCeilF(80.0f, camera->fov, 1.0f / *timer, 0.1f);
...
s32 Camera_Special5(Camera* camera) {
    ...
    camera->fov = Camera_SmoothStepToCeilF(roData->fovTarget, camera->fov,
...
s32 Camera_Special7(Camera* camera) {
    ...
    if (camera->at.y < sCamElevatorPlatformTogglePosY[rwData->index]) {
        ...
        atTarget.y -= 20.0f;
        Camera_SmoothStepToCeilVec3f(&atTarget, &camera->at, 0.4f, 0.4f, 0.10f);
        ...
        atTarget = playerPosRot->pos;
        atTarget.y += yOffset;
        Camera_SmoothStepToCeilVec3f(&atTarget, &camera->at, 0.4f, 0.4f, 0.1f);
...
s32 Camera_Special6(Camera* camera) {
    ...
    if (rwData->animTimer > 0) {
        ...
        eyeAnim = eyePosCalc;
        eyeAnim.y = Camera_SmoothStepToCeilF(eyePosCalc.y, eye->y, 0.5f, 0.01f);
        ...
        eyePosCalc.y += (playerPosRot->pos.y - rwData->initalPlayerY) * 0.2f;
        *eye = eyePosCalc;
        eye->y = Camera_SmoothStepToCeilF(eyePosCalc.y, eye->y, 0.5f, 0.01f);
...
s32 Camera_Special9(Camera* camera) {
    ...
    switch (camera->animState) {
        ...
        case 2:
            ...
            Camera_SmoothStepToCeilVec3f(&spAC, at, 0.25f, 0.25f, 0.1f);
            doorParams->timer2--;
            if (doorParams->timer2 <= 0) {
                camera->animState++;
        ...
        case 3:
            spAC = playerPosRot->pos;
            spAC.y += (playerYOffset + roData->yOffset);
            Camera_SmoothStepToCeilVec3f(&spAC, at, 0.5f, 0.5f, 0.1f);
            eyeAdjustment.pitch = Camera_SmoothStepToCeilS(0xAAA, atEyeOffsetGeo.pitch, 0.3f, 0xA);
            eyeAdjustment.yaw = Camera_SmoothStepToCeilS(rwData->targetYaw, atEyeOffsetGeo.yaw, 0.3f, 0xA);
            eyeAdjustment.r = Camera_SmoothStepToCeilF(60.0f, atEyeOffsetGeo.r, 0.3f, 1.0f);
            Camera_AddVecGeoToVec3f(eyeNext, at, &eyeAdjustment);
            *eye = *eyeNext;
            doorParams->timer3--;
            if (doorParams->timer3 <= 0) {
