...
Acmd* AudioSynth_ProcessEnvelope(Acmd* cmd, NoteSubEu* noteSubEu, NoteSynthesisState* synthState, s32 aiBufLen,
                                 u16 dmemSrc, s32 haasEffectSide, s32 flags);
...
Acmd* AudioSynth_ProcessNote(s32 noteIndex, NoteSubEu* noteSubEu, NoteSynthesisState* synthState, s16* aiBuf,
    ...
    if ((noteSubEu->haasEffectLeftDelaySize != 0) || (synthState->prevHaasEffectLeftDelaySize != 0)) {
        haasEffectSide = HAAS_EFFECT_DELAY_LEFT;
    } else if ((noteSubEu->haasEffectRightDelaySize != 0) || (synthState->prevHaasEffectRightDelaySize != 0)) {
        haasEffectSide = HAAS_EFFECT_DELAY_RIGHT;
    } else {
        haasEffectSide = HAAS_EFFECT_DELAY_NONE;
    }
    cmd = AudioSynth_ProcessEnvelope(cmd, noteSubEu, synthState, aiBufLen, DMEM_TEMP, haasEffectSide, flags);
    if (noteSubEu->bitField1.usesHeadsetPanEffects2) {
        if (!(flags & A_INIT)) {
            flags = A_CONTINUE;
        }
        cmd = AudioSynth_ApplyHaasEffect(cmd, noteSubEu, synthState, aiBufLen * 2, flags, haasEffectSide);
    }
    return cmd;
}
...
Acmd* AudioSynth_ProcessEnvelope(Acmd* cmd, NoteSubEu* noteSubEu, NoteSynthesisState* synthState, s32 aiBufLen,
    ...
    srcReverbVolAndFlags = synthState->reverbVol;
    srcReverbVol = srcReverbVolAndFlags & 0x7F;

    if (srcReverbVolAndFlags != reverbVol) {
        rampReverb = (((reverbVol & 0x7F) - srcReverbVol) << 9) / (aiBufLen >> 3);
    ...
    if (noteSubEu->bitField1.usesHeadsetPanEffects2) {
        ...
        AudioSynth_EnvSetup1(cmd++, srcReverbVol * 2, rampReverb, rampLeft, rampRight);
        AudioSynth_EnvSetup2(cmd++, curVolLeft, curVolRight);

        switch (haasEffectSide) {
        ...
        aEnvSetup1(cmd++, srcReverbVol * 2, rampReverb, rampLeft, rampRight);
        aEnvSetup2(cmd++, curVolLeft, curVolRight);
        dmemDest = sEnvMixerDefaultDmem;
    }

    aEnvMixer(cmd++, dmemSrc, aiBufLen, (srcReverbVolAndFlags & 0x80) >> 7, noteSubEu->bitField0.envMixerNegWetLeft,
              noteSubEu->bitField0.envMixerNegWetRight, noteSubEu->bitField0.envMixerNegDryLeft,
              noteSubEu->bitField0.envMixerNegDryRight, dmemDest, sEnvMixerOp);
    return cmd;
}
