...
u8 Message_ShouldAdvance(PlayState* play) {
    ...
    if (CHECK_BTN_ALL(input->press.button, BTN_A) || CHECK_BTN_ALL(input->press.button, BTN_B) ||
        ...
        AudioSfx_PlaySfx(SFX_ID_SYSTEM_MESSAGE_PASS, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
...
void Message_CloseTextbox(PlayState* play) {
    ...
    if (msgCtx->msgLength != 0) {
        ...
        AudioSfx_PlaySfx(SFX_ID_NONE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultReverb);
    }
}
...
void Message_HandleChoiceSelection(PlayState* play, u8 numChoices) {
    ...
    if (input->rel.stick_y >= 30 && !sAnalogStickHeld) {
        ...
        if (msgCtx->choiceIndex > 128) {
            ...
            AudioSfx_PlaySfx(SFX_ID_SYSTEM_CURSOR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        }
...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** p, u16 i) {
    ...
    // clang-format off
    if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) { AudioSfx_PlaySfx(SFX_ID_NONE, &gSfxDefaultPos, 4,
        ...
        switch (character) {
            ...
            case MESSAGE_BOX_BREAK:
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) {
                    if (!sTextboxSkipped) {
                        AudioSfx_PlaySfx(SFX_ID_NONE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            ...
            case MESSAGE_SFX:
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING && !sMessageHasSetSfx) {
                    ...
                    AudioSfx_PlaySfx(sfxHi | msgCtx->msgBufDecoded[i + 2], &gSfxDefaultPos, 4,
            ...
            case MESSAGE_END:
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) {
                    ...
                    if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_DEFAULT) {
                        AudioSfx_PlaySfx(SFX_ID_SYSTEM_MESSAGE_END, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
    ...
    if (msgCtx->msgLength != 0) {
        ...
        switch (msgCtx->msgMode) {
                    ...
                    if (msgCtx->ocarinaStaff->state == OCARINA_SONG_SCARECROW_SPAWN ||
                        ...
                        if (msgCtx->ocarinaAction == OCARINA_ACTION_CHECK_NOWARP) {
                            if (msgCtx->ocarinaStaff->state < OCARINA_SONG_SARIAS ||
                                ...
                                AudioSfx_PlaySfx(SFX_ID_SYSTEM_OCARINA_ERROR, &gSfxDefaultPos, 4,
                                ...
                                AudioSfx_PlaySfx(SFX_ID_SYSTEM_TRE_BOX_APPEAR, &gSfxDefaultPos, 4,
                                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                                                 &gSfxDefaultReverb);
                                Interface_ChangeAlpha(1);
                            }
                        ...
                        AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_OFF);
                        AudioSfx_PlaySfx(SFX_ID_SYSTEM_OCARINA_ERROR, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                        msgCtx->msgMode = MSGMODE_OCARINA_STARTING;
                    }
            ...
            case MSGMODE_MEMORY_GAME_RIGHT_SKULLKID_PLAYING:
                AudioSfx_PlaySfx(SFX_ID_SYSTEM_METRONOME_LV - SFX_FLAG, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                msgCtx->ocarinaStaff = AudioOcarina_GetPlaybackStaff();
                if (msgCtx->ocarinaStaff->pos && sOcarinaButtonIndexBufPos == msgCtx->ocarinaStaff->pos - 1) {
                    sOcarinaButtonIndexBuf[msgCtx->ocarinaStaff->pos - 1] = msgCtx->ocarinaStaff->buttonIndex;
                ...
                if (msgCtx->stateTimer == 0) {
                    if (msgCtx->ocarinaStaff->state == 0) {
                        if (msgCtx->msgMode == MSGMODE_MEMORY_GAME_LEFT_SKULLKID_PLAYING) {
                            AudioSfx_PlaySfx(SFX_ID_SYSTEM_METRONOME, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                        } else {
                            AudioSfx_PlaySfx(SFX_ID_SYSTEM_METRONOME_2, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                        }
                        msgCtx->msgMode++;
                    }
            ...
            case MSGMODE_MEMORY_GAME_START_NEXT_ROUND:
                if (!AudioSfx_IsPlaying(SFX_ID_SYSTEM_METRONOME)) {
                    msgCtx->ocarinaStaff = AudioOcarina_GetPlaybackStaff();
                    msgCtx->ocarinaStaff->pos = sOcarinaButtonIndexBufPos = 0;
                    Message_ResetOcarinaNoteState();
                    AudioOcarina_SetPlaybackSong(OCARINA_SONG_MEMORY_GAME + 1, 1);
        ...
        case MSGMODE_TEXT_START:
            ...
            if (var) {
                ...
                if (msgCtx->textBoxType == TEXTBOX_TYPE_NONE_BOTTOM ||
                    ...
                    Audio_PlaySfx_IfNotInCutscene(SFX_ID_NONE);
                    msgCtx->stateTimer = 0;
            ...
            if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_FADING) {
                ...
                if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_2_CHOICE && play->msgCtx.ocarinaMode == OCARINA_MODE_01) {
                    ...
                    if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_HAS_NEXT) {
                        ...
                        AudioSfx_PlaySfx(SFX_ID_SYSTEM_DECIDE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                        Message_CloseTextbox(play);
                    }
                }
