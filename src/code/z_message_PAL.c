...
static u16 sOcarinaAvailableSongs = 0; // ocarina bit flags
...
void Message_UpdateOcarinaMemoryGame(PlayState* play) {
    ...
    if (play->msgCtx.msgMode == MSGMODE_MEMORY_GAME_PROMPT) {
        AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
        msgCtx->ocarinaStaff = AudioOcarina_GetPromptStaff();
        msgCtx->ocarinaStaff->pos = sOcarinaButtonIndexBufPos = 0;
        AudioOcarina_Start((1 << OCARINA_SONG_MEMORY_GAME) + OCARINA_START_ONE_NOTE_LIMIT);
        msgCtx->textDrawPos = msgCtx->decodedTextLen;
    } else if (msgCtx->msgMode == MSGMODE_MEMORY_GAME_RIGHT_SKULLKID_DEMONSTRATION) {
...
void Message_DrawTextChar(PlayState* play, void* textureImage, Gfx** gfxP) {
    MessageContext* msgCtx = &play->msgCtx;
    Gfx* gfx = *gfxP;
...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** gfxP, u16 i) {
    s32 pad;
    Gfx* gfx = *gfxP;
    MessageContext* msgCtx = &play->msgCtx;
...
void Message_HandleOcarina(PlayState* play) {
    ...
    if (play->msgCtx.msgMode == MSGMODE_TEXT_DISPLAYING) {
        ...
        if (msgCtx->ocarinaAction == OCARINA_ACTION_SCARECROW_LONG_RECORDING) {
            ...
            AudioOcarina_SetDemonstrationSong(OCARINA_SONG_SCARECROW_SPAWN + 1, 1);
...
void Message_ContinueTextbox(PlayState* play, u16 textId) {
    ...
    if (!R_IS_SHOPPING && (play->interfaceCtx.unk_1FA == 0)) {
        Interface_SetDoAction(play, DO_ACTION_NEXT);
    }
    msgCtx->textboxColorAlphaCurrent = msgCtx->textboxColorAlphaTarget;
}

void Message_DisplayOcarinaStaffImpl(PlayState* play, u16 ocarinaActionId) {
    ...
    for (i = sOcarinaAvailableSongs = 0; i <= (QUEST_SONG_STORMS - QUEST_SONG_MINUET); i++) {
        if (CHECK_QUEST_ITEM(QUEST_SONG_MINUET + i)) {
            osSyncPrintf("ocarina_check_bit[%d]=%x\n", i, sOcarinaSongFlagsMap[i]);
            sOcarinaAvailableSongs |= sOcarinaSongFlagsMap[i];
        }
    }
    ...
    Message_ResetOcarinaNoteState();
    sLastPlayedSong = msgCtx->lastPlayedSongAlt = msgCtx->lastOcarinaButtonIndex = 0xFF;
    ...
    if (ocarinaActionId >= 0x893) {
        ...
        if (ocarinaActionId == OCARINA_ACTION_SCARECROW_SPAWN_DEMONSTRATION) {
    ...
    if ((ocarinaActionId == OCARINA_ACTION_FREE_PLAY) || (ocarinaActionId == OCARINA_ACTION_CHECK_NOWARP)) {
        ...
        Message_Decode(play);
        msgCtx->msgMode = MSGMODE_MEMORY_GAME_START;
    } else if (ocarinaActionId == OCARINA_ACTION_SCARECROW_LONG_DEMONSTRATION) {
        // "?????Recording Playback / Recording Playback / Recording Playback / Recording Playback -> "
        osSyncPrintf("?????録音再生 録音再生 録音再生 録音再生  -> ");
        AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
        AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
...
void Message_DisplayOcarinaStaff(PlayState* play, u16 ocarinaActionId) {
    play->msgCtx.blockSunsSongEffect = false;
    Message_DisplayOcarinaStaffImpl(play, ocarinaActionId);
}

void Message_DisplayOcarinaStaffBlockSunsSongEffect(PlayState* play, u16 ocarinaActionId) {
    play->msgCtx.blockSunsSongEffect = true;
    Message_DisplayOcarinaStaffImpl(play, ocarinaActionId);
}
    ...
    if (msgCtx->msgLength != 0) {
        ...
        switch (msgCtx->msgMode) {
                ...
                if (msgCtx->msgMode == MSGMODE_OCARINA_STARTING) {
                    if ((msgCtx->ocarinaAction == OCARINA_ACTION_UNK_0) ||
                        ...
                        if ((msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY) ||
                            (msgCtx->ocarinaAction == OCARINA_ACTION_CHECK_NOWARP)) {
                            AudioOcarina_Start(sOcarinaAvailableSongs + OCARINA_START_NO_NOTE_LIMIT);
                        } else {
                            // "On Stage Performance"
                            osSyncPrintf("台上演奏\n");
                            AudioOcarina_Start(sOcarinaAvailableSongs + OCARINA_START_EIGHT_NOTE_LIMIT);
                        }
                    } else {
                        osSyncPrintf("Na_StartOcarinaSinglePlayCheck2( message->ocarina_no );\n");
                        AudioOcarina_Start((1 << msgCtx->ocarinaAction) + OCARINA_START_ONE_NOTE_LIMIT);
                    }
            ...
            case MSGMODE_SETUP_DISPLAY_SONG_PLAYED:
                ...
                AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
                AudioOcarina_SetInstrument(OCARINA_INSTRUMENT_DEFAULT);
                AudioOcarina_SetDemonstrationSong(msgCtx->lastPlayedSong + 1, 1);
            ...
            case MSGMODE_SONG_PLAYED_ACT:
                // A song has been played, process the result
                msgCtx->stateTimer--;
                if (msgCtx->stateTimer == 0) {
                    if ((msgCtx->lastPlayedSong <= OCARINA_SONG_PRELUDE) &&
                        !((msgCtx->ocarinaAction >= OCARINA_ACTION_PROMPT_MINUET) &&
                          (msgCtx->ocarinaAction <= OCARINA_ACTION_PROMPT_PRELUDE))) {
                        // Played a warp song that was not a prompt
                        ...
                        if (msgCtx->lastPlayedSong == OCARINA_SONG_EPONAS) {
                            R_LAST_PLAYED_EPONAS_SONG = true;
                        }
                        osSyncPrintf(VT_FGCOL(YELLOW));
                        ...
                        if (msgCtx->ocarinaAction == OCARINA_ACTION_FREE_PLAY_DONE) {
                            ...
                            if ((msgCtx->lastPlayedSong + OCARINA_ACTION_CHECK_MINUET) == msgCtx->ocarinaAction) {
                                // The song played was the song being checked for
                                play->msgCtx.ocarinaMode = OCARINA_MODE_EVENT;
                            } else {
                                // The song played was not the song being checked for
                                play->msgCtx.ocarinaMode = msgCtx->lastPlayedSong - 1;
                            }
                        } else {
                            // Process prompted songs
                            osSyncPrintf(VT_FGCOL(GREEN));
            ...
            case MSGMODE_MEMORY_GAME_START:
                ...
                Message_ResetOcarinaNoteState();
                AudioOcarina_SetDemonstrationSong(OCARINA_SONG_MEMORY_GAME + 1, 1);
                msgCtx->msgMode = MSGMODE_MEMORY_GAME_LEFT_SKULLKID_DEMONSTRATION;
                msgCtx->stateTimer = 2;
                break;

            case MSGMODE_MEMORY_GAME_LEFT_SKULLKID_DEMONSTRATION:
            case MSGMODE_MEMORY_GAME_RIGHT_SKULLKID_DEMONSTRATION:
            ...
            case MSGMODE_FROGS_START:
                ...
                Message_ResetOcarinaNoteState();
                AudioOcarina_Start(sOcarinaAvailableSongs + OCARINA_START_NO_NOTE_LIMIT);
                msgCtx->msgMode = MSGMODE_FROGS_PROMPT;
                break;
...
void Message_DrawDebugText(PlayState* play, Gfx** gfxP) {
    s32 pad;
    GfxPrint printer;
    s32 pad1;
...
void Message_Update(PlayState* play) {
    ...
    switch (msgCtx->msgMode) {
        ...
        case MSGMODE_TEXT_DISPLAYING:
            if ((msgCtx->textBoxType != TEXTBOX_TYPE_NONE_BOTTOM) && !R_IS_SHOPPING &&
        ...
        case MSGMODE_TEXT_AWAIT_INPUT:
            if (!R_IS_SHOPPING && Message_ShouldAdvance(play)) {
                msgCtx->msgMode = MSGMODE_TEXT_DISPLAYING;
                msgCtx->textDrawPos++;
            }
            break;
            ...
            if (msgCtx->textboxEndType == TEXTBOX_ENDTYPE_FADING) {
                ...
                if (msgCtx->stateTimer == 0) {
                    Message_CloseTextbox(play);
                       ...
                       (msgCtx->textboxEndType != TEXTBOX_ENDTYPE_EVENT) && !R_IS_SHOPPING) {
        ...
        case MSGMODE_TEXT_CLOSING:
            ...
            osSyncPrintf("OCARINA_MODE=%d   chk_ocarina_no=%d\n", play->msgCtx.ocarinaMode, msgCtx->lastPlayedSongAlt);
            break;
