...
void AudioLoad_DecreaseSampleChunkTtls(void) {
    ...
    gAudioContext.sampleChunkUnused = 0;
}
...
void* AudioLoad_AllocSampleChunkCache(u32 devAddr, u32 size, s32 sampleFlags, u8* prevSampleChunkIndex, s32 medium) {
            ...
            if ((offsetToEntry >= 0) && ((u32)offsetToEntry <= (entry->size - size))) {
                ...
                if (entry->ttl == 0) {
                    ...
                    if (entry->reuseIndex != gAudioContext.shortTtlSampleChunkReuseRdPos) {
                        ...
                        gAudioContext
                            .sampleChunkEntries[gAudioContext.shortTtlSampleChunkReuseQueue
    ...
    AudioLoad_Dma(&gAudioContext.SampleChunkCacheIoMsg[gAudioContext.sampleChunkDmaCount++], OS_MESG_PRI_NORMAL,
                  OS_READ, dmaDevAddr, entry->ramAddr, dmaSize, &gAudioContext.SampleChunkCacheMsgQueue, medium,
                  "SUPERDMA");
...
void AudioLoad_InitSampleChunkCache(s32 numNotes) {
    ...
    gAudioContext.sampleChunkEntries =
        AudioHeap_Alloc(&gAudioContext.miscPool, 4 * gAudioContext.numNotes * sizeof(SampleChunkCacheEntry) *
    ...
    for (i = 0; i < numShortTtlSampleChunksMax; i++) {
        ...
        entry->ramAddr = AudioHeap_AllocAttemptExternal(&gAudioContext.miscPool, gAudioContext.sampleChunkEntrySize);
...
void AudioLoad_Init(void* heap, u32 heapSize) {
    ...
    osCreateMesgQueue(&gAudioContext.SampleChunkCacheMsgQueue, gAudioContext.SampleChunkCacheMsgBuf,
                      ARRAY_COUNT(gAudioContext.SampleChunkCacheMsgBuf));
