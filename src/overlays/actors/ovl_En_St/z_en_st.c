...
void EnSt_SetReturnToCeilingAnimation(EnSt* this) {
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_UP);
...
s32 EnSt_CheckHitPlayer(EnSt* this, PlayState* play) {
    ...
    if (this->swayTimer == 0) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_ROLL);
    }
...
void EnSt_Sway(EnSt* this) {
    ...
    if (this->swayTimer != 0) {
        ...
        if (this->absPrevSwayAngle >= ABS(rotAngle) && this->playSwayFlag == 0) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_WAVE);
            this->playSwayFlag = 1;
        }
...
void EnSt_WaitOnGround(EnSt* this, PlayState* play) {
    ...
    if (DECR(this->sfxTimer) == 0) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_LAUGH);
        this->sfxTimer = 64;
    }
...
void EnSt_LandOnGround(EnSt* this, PlayState* play) {
    ...
    if (this->sfxTimer == 14) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_DOWN_SET);
    }
...
void EnSt_MoveToGround(EnSt* this, PlayState* play) {
    ...
    if (!EnSt_IsCloseToPlayer(this, play)) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALTU_DOWN);
        this->sfxTimer = 3;
    }
}
