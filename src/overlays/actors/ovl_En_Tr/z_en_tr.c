...
void EnTr_CrySpellcast(EnTr* this, PlayState* play) {
    if (this->timer == 11) {
        ...
        if ((this->actor.params != TR_KOUME) || (gSaveContext.sceneLayer == 6)) {
            AudioSfx_PlaySfx(SFX_ID_ENEMY_TWINROBA_SHOOT_VOICE, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
    ...
    Actor_PlaySfx_Flagged(&this->actor, SFX_ID_ENEMY_TWINROBA_FLY_DEMO - SFX_FLAG);
}
