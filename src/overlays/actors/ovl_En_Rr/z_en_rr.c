...
void EnRr_SetSpeed(EnRr* this, f32 speed) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_WALK);
}
...
void EnRr_SetupReach(EnRr* this) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_UNARI);
}
...
void EnRr_SetupGrabPlayer(EnRr* this, Player* player) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_DRINK);
}
...
void EnRr_SetupReleasePlayer(EnRr* this, GlobalContext* globalCtx) {
    ...
    if (this->actor.colorFilterTimer == 0) {
        this->actionFunc = EnRr_Approach;
        Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_THROW);
...
void EnRr_SetupDamage(EnRr* this) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_DAMAGE);
}
...
void EnRr_SetupDeath(EnRr* this) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_DEAD);
    this->actor.flags &= ~ACTOR_FLAG_0;
}
...
void EnRr_GrabPlayer(EnRr* this, GlobalContext* globalCtx) {
    ...
    if ((this->frameCount % 8) == 0) {
        Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_LIKE_EAT);
    }
    this->ocTimer = 8;
