...
void EnRr_SetSpeed(EnRr* this, f32 speed) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_WALK);
}
...
void EnRr_SetupReach(EnRr* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_UNARI);
}
...
void EnRr_SetupGrabPlayer(EnRr* this, Player* player) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_DRINK);
}
...
void EnRr_SetupReleasePlayer(EnRr* this, PlayState* play) {
    ...
    if (this->actor.colorFilterTimer == 0) {
        this->actionFunc = EnRr_Approach;
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_THROW);
...
void EnRr_SetupDamage(EnRr* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_DAMAGE);
}
...
void EnRr_SetupDeath(EnRr* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_DEAD);
    this->actor.flags &= ~ACTOR_FLAG_0;
}
...
void EnRr_GrabPlayer(EnRr* this, PlayState* play) {
    ...
    if ((this->frameCount % 8) == 0) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_LIKE_EAT);
    }
    this->ocTimer = 8;
