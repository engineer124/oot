...
void BgIceTurara_Break(BgIceTurara* this, PlayState* play, f32 arg2) {
    ...
    SfxSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.world.pos, 30, SFX_ID_ENVIRONMENT_ICE_BROKEN);
    for (i = 0; i < 2; i++) {
        for (j = 0; j < 10; j++) {
...
void BgIceTurara_Shiver(BgIceTurara* this, PlayState* play) {
    ...
    if (!(this->shiverTimer % 4)) {
        Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENVIRONMENT_ICE_SWING);
    }
    if (this->shiverTimer == 0) {
