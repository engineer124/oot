...
#define FLAGS (ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_NO_UPDATE_CULLING | ACTOR_FLAG_CAN_HOLD_SWITCH)
...
void func_80AEBAFC(EnRu1* this) {
    if (this->unk_298 == 0) {
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_EV_DIVE_INTO_WATER);
        this->unk_298 = 1;
    }
}
...
void func_80AEBB3C(EnRu1* this) {
    if (Animation_OnFrame(&this->skelAnime, 5.0f)) {
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_PL_FACE_UP);
    }
}
...
void func_80AEBB78(EnRu1* this) {
    ...
    if (Animation_OnFrame(skelAnime, 4.0f) || Animation_OnFrame(skelAnime, 13.0f) ||
        ...
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_PL_SWIM);
    }
}

void func_80AEBBF4(EnRu1* this) {
    if (Animation_OnFrame(&this->skelAnime, 8.0f)) {
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_PL_SUBMERGE);
    }
}
...
void func_80AEBC84(EnRu1* this, PlayState* play) {
    if (play->csCtx.curFrame == 0x82) {
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_LAUGH_0);
    }
}
...
void func_80AEC650(EnRu1* this) {
    ...
    if (this->unk_280 == 0) {
        if (Animation_OnFrame(&this->skelAnime, 2.0f) || Animation_OnFrame(&this->skelAnime, 7.0f)) {
            Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_PL_WALK_GROUND + SURFACE_SFX_OFFSET_JABU);
        }
    }
}

void func_80AEC6B0(EnRu1* this) {
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_EV_FALL_DOWN_DIRT);
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_FALL);
}
...
void func_80AEC780(EnRu1* this, PlayState* play) {
    ...
    if ((func_80AEC5FC(this, play)) && (!Play_InCsMode(play)) &&
        (!(player->stateFlags1 &
           (PLAYER_STATE1_HANGING_FROM_LEDGE_SLIP | PLAYER_STATE1_CLIMBING_ONTO_LEDGE | PLAYER_STATE1_CLIMBING))) &&
...
void func_80AED4FC(EnRu1* this) {
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_EV_LAND_DIRT);
}
...
void func_80AED520(EnRu1* this, PlayState* play) {
    ...
    Audio_PlaySfxGeneral(NA_SE_PL_PULL_UP_RUTO, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_LIFT);
}

void func_80AED57C(EnRu1* this) {
    if (this->actor.speed != 0.0f) {
        Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_THROW);
    }
}

void func_80AED5B8(EnRu1* this) {
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_CRASH);
}

void func_80AED5DC(EnRu1* this) {
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_UNBALLANCE);
}

void func_80AED600(EnRu1* this) {
    Audio_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_VO_RT_DISCOVER);
}
