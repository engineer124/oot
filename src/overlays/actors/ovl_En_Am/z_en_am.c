...
void EnAm_SpawnEffects(EnAm* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_AMOS_WALK);
...
void EnAm_SetupRecoilFromDamage(EnAm* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_AMOS_DAMAGE);
...
void EnAm_Sleep(EnAm* this, PlayState* play) {
    ...
    if ((this->unk_258 != 0) ||
        ...
        if (this->textureBlend == 0) {
            Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_AMOS_WAVE);
            Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_AMOS_VOICE);
        ...
        if (this->textureBlend > 10) {
            ...
            if (this->dyna.actor.speedXZ != 0.0f) {
                Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENVIRONMENT_ROCK_SLIDE - SFX_FLAG);
            }
...
void EnAm_SetupStunned(EnAm* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_GOMA_JR_FREEZE);
    ...
    if (this->dyna.actor.colChkInfo.damageEffect != AM_DMGEFF_MAGIC_FIRE_LIGHT) {
        ...
        if (this->deathTimer != 0) {
            ...
            if (this->deathTimer == 0) {
                ...
                Actor_PlaySfx(&this->dyna.actor, SFX_ID_ENEMY_AMOS_DEAD);
