...
void EnDekubaba_SetupGrow(EnDekubaba* this) {
    ...
    this->collider.base.acFlags &= ~AC_HARD;
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DUMMY482);
...
void EnDekubaba_DecideLunge(EnDekubaba* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 0.0f) || Animation_OnFrame(&this->skelAnime, 12.0f)) {
        if (this->actor.params == DEKUBABA_BIG) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_MOUTH);
        } else {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_JR_MOUTH);
        }
    }
...
void EnDekubaba_Lunge(EnDekubaba* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        if (Animation_OnFrame(&this->skelAnime, 1.0f)) {
            if (this->actor.params == DEKUBABA_BIG) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_ATTACK);
            } else {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_JR_ATTACK);
            }
        }
...
void EnDekubaba_PullBack(EnDekubaba* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        ...
        if (this->timer == 10) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_SCRAPE);
        }
...
void EnDekubaba_UpdateDamage(EnDekubaba* this, PlayState* play) {
    ...
    if (this->actor.colChkInfo.health != 0) {
        if (this->timer == 2) {
            ...
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_DAMAGE);
        }
    } else {
        Enemy_StartFinishingBlow(play, &this->actor);
        if (this->actor.params == DEKUBABA_BIG) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_DEAD);
        } else {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEKU_JR_DEAD);
        }
    }
}
