...
void BossGanon_IntroCutscene(BossGanon* this, PlayState* play) {
    ...
    switch (this->csState) {
        ...
        case 2:
            ...
            if (this->csTimer == 13) {
                Player_PlaySfx(player, player->ageProperties->voiceSfxIdOffset + NA_SE_VO_LI_SURPRISE);
            }
        ...
        case 6:
            ...
            Audio_PlaySfx(NA_SE_EV_TRIFORCE_MARK);
            play->envCtx.lightBlend = 0.0f;
            FALLTHROUGH;
        case 7:
            this->envLightMode = 6;
    ...
    if (this->unk_1C2 == 0) {
        ...
        if (sqrtf(SQ(xDiff) + SQ(zDiff) + SQ(yDiff)) < 40.0f) {
            ...
            if (this->actor.params == 0xC8) {
                Audio_PlaySfx(NA_SE_EN_GANON_DAMAGE2);
                Audio_PlaySfx(NA_SE_EN_GANON_DD_THUNDER);
    ...
    for (i = 0; i < BOSSGANON_EFFECT_COUNT; i++, eff++) {
        if (eff->type != GDF_EFF_NONE) {
            ...
            if (eff->type == GDF_EFF_WINDOW_SHARD) {
                ...
                if (i == 0) {
                    Audio_PlaySfx(NA_SE_EN_GANON_WAVE_GND - SFX_FLAG);
                }
