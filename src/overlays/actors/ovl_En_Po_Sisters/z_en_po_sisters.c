...
#define FLAGS                                                                                                   \
    (ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_UNFRIENDLY | ACTOR_FLAG_NO_UPDATE_CULLING | ACTOR_FLAG_HOOK_CAN_CARRY | \
     ACTOR_FLAG_IGNORE_QUAKE | ACTOR_FLAG_ARROW_CAN_CARRY)
...
void EnPoSisters_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params & 0x1000) {
        ...
        if (this->unk_195 == 0) {
            ...
            this->actor.flags &= ~(ACTOR_FLAG_HOOK_CAN_CARRY | ACTOR_FLAG_ARROW_CAN_CARRY);
