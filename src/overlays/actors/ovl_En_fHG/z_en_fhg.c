    ...
    switch (this->cutsceneState) {
        ...
        case INTRO_START:
            if (GET_EVENTCHKINF(EVENTCHKINF_72)) {
                ...
                if (this->timers[0] == 51) {
                    Actor_PlaySfx(this->actor.child, SFX_ID_ENVIRONMENT_SPEAR_FENCE);
        ...
        case INTRO_BACK:
            if (this->timers[0] == 25) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_GANON_HORSE_GROAN);
            }
            if (this->timers[0] == 20) {
        ...
        case INTRO_LAUGH:
            ...
            if (this->timers[0] == 35) {
                Lib_PlaySfx_AtPos(&audioVec, SFX_ID_ENEMY_FANTOM_EYE);
            }
            if (this->timers[0] == 130) {
                bossGnd->work[GND_EYE_STATE] = GND_EYESTATE_FADE;
                Lib_PlaySfx_AtPos(&audioVec, SFX_ID_ENEMY_FANTOM_ST_LAUGH);
            }
            if (this->timers[0] == 20) {
                SEQCMD_PLAY_SEQUENCE(SEQ_PLAYER_BGM_MAIN, 0, 0, SEQ_ID_BOSS);
            }
...
void EnfHG_Approach(EnfHG* this, PlayState* play) {
    ...
    if (this->actor.params == GND_REAL_BOSS) {
        ...
        if ((this->gallopTimer % 8) == 0) {
            Lib_PlaySfx_AtPos(&this->hoofSfxPos, SFX_ID_ENVIRONMENT_HORSE_RUN);
        }
    }
    SkelAnime_Update(&this->skin.skelAnime);
