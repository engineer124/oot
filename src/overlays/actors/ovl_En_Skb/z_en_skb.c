...
void EnSkb_SetupRiseFromGround(EnSkb* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_RIVA_APPEAR);
...
void EnSkb_SetupDespawn(EnSkb* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_AKINDONUTS_HIDE);
...
void EnSkb_WalkForward(EnSkb* this, PlayState* play) {
    ...
    if (thisKeyFrame != (s32)this->skelAnime.curFrame) {
        if (((prevKeyFrame < 9) && (((s32)playSpeed + thisKeyFrame) >= 8)) ||
            ...
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALKID_WALK);
        }
    }
...
void EnSkb_SetupTakeDamage(EnSkb* this) {
    ...
    this->actor.world.rot.y = this->actor.yawTowardsPlayer;
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_STALKID_DAMAGE);
...
void EnSkb_SetupDeath(EnSkb* this, PlayState* play) {
    ...
    this->breakFlags |= 4;
    EffectSsDeadSound_SpawnStationary(play, &this->actor.projectedPos, SFX_ID_ENEMY_STALKID_DEAD, 1, 1, 0x28);
