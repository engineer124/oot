...
void EnWallmas_WaitToDrop(EnWallmas* this, PlayState* play) {
    ...
    if ((player->stateFlags1 & PLAYER_STATE1_20) || (player->stateFlags1 & PLAYER_STATE1_27) ||
        ...
        AudioSfx_StopById(SFX_ID_ENEMY_FALL_AIM);
        this->timer = 0x82;
    }

    if (this->timer == 0x50) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_AIM);
    }
...
void EnWallmas_ReturnToCeiling(EnWallmas* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 20.0f) != 0) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_UP);
    }
...
void EnWallmas_TakePlayer(EnWallmas* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 1.0f) != 0) {
        if (!LINK_IS_ADULT) {
            Player_PlaySfx(&this->actor, SFX_ID_VOICE_LI_DAMAGE_S_KID);
        } else {
            Player_PlaySfx(&this->actor, SFX_ID_VOICE_LI_DAMAGE_S);
        }

        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_CATCH);
    }
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->timer == -0x1E) {
            if (!LINK_IS_ADULT) {
                Player_PlaySfx(&this->actor, SFX_ID_VOICE_LI_TAKEN_AWAY_KID);
            } else {
                Player_PlaySfx(&this->actor, SFX_ID_VOICE_LI_TAKEN_AWAY);
            }
        }
        if (this->timer == 0) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_UP);
