...
void EnFd_SpinAndSpawnFire(EnFd* this, PlayState* play) {
    ...
    if ((this->spinTimer < 31) && (this->invincibilityTimer == 0)) {
        Actor_PlaySfx_Shared(&this->actor, NA_SE_EN_FLAME_FIRE_ATTACK - SFX_FLAG);
    } else {
        Actor_PlaySfx_Shared(&this->actor, NA_SE_EN_FLAME_ROLL - SFX_FLAG);
    }
...
void EnFd_Run(EnFd* this, PlayState* play) {
    ...
    Actor_PlaySfx_Shared(&this->actor, NA_SE_EN_FLAME_RUN - SFX_FLAG);
