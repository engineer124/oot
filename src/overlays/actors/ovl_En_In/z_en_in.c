...
void func_80A7A304(EnIn* this, PlayState* play) {
    if (this->skelAnime.animation == &object_in_Anim_015814 || this->skelAnime.animation == &object_in_Anim_01646C) {
        if (this->skelAnime.curFrame == 8.0f) {
            Audio_PlaySfx_Randomized(&this->actor.projectedPos, SFX_ID_VOICE_IN_LASH_0,
                                     SFX_ID_VOICE_IN_LASH_1 - SFX_ID_VOICE_IN_LASH_0 + 1);
        }
    }
    if (this->skelAnime.animation == &object_in_Anim_018C38 && this->skelAnime.curFrame == 20.0f) {
        Actor_PlaySfx(&this->actor, SFX_ID_VOICE_IN_CRY_0);
    }
    if (SkelAnime_Update(&this->skelAnime)) {
        this->animationIdx %= 8;
        this->unk_1E8 = this->animationIdx;
        if (this->animationIdx == 3 || this->animationIdx == 4) {
            AudioSfx_PlaySfx(SFX_ID_ITEM_LASH, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            if (Rand_ZeroOne() < 0.3f) {
                AudioSfx_PlaySfx(SFX_ID_ITEM_INGO_HORSE_NEIGH, &this->actor.projectedPos, 4,
...
void func_80A7A568(EnIn* this, PlayState* play) {
    ...
    if (gSaveContext.timerState == TIMER_STATE_STOP) {
        AudioSfx_PlaySfx(SFX_ID_SYSTEM_FOUND, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
...
void func_80A7A940(EnIn* this, PlayState* play) {
    ...
    if (this->unk_1EC != 0) {
        this->unk_1EC--;
        if (this->unk_1EC == 0) {
            Actor_PlaySfx(&this->actor, SFX_ID_VOICE_IN_LOST);
        }
    }
    if (this->unk_308.unk_00 == 2) {
        this->actor.flags &= ~ACTOR_FLAG_16;
...
void func_80A7ABD4(EnIn* this, PlayState* play) {
    ...
    if (this->unk_308.unk_00 != 0) {
        ...
        if (play->csCtx.frames == 44) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_RONRON_DOOR_CLOSE);
        }
