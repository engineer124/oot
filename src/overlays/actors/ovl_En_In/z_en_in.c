...
void func_80A7A304(EnIn* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->animationIdx == 3 || this->animationIdx == 4) {
            AudioSfx_PlaySfx(NA_SE_IT_LASH, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            if (Rand_ZeroOne() < 0.3f) {
                AudioSfx_PlaySfx(NA_SE_IT_INGO_HORSE_NEIGH, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            }
        }
        Animation_Change(&this->skelAnime, D_80A7B918[this->animationIdx], 1.0f, 0.0f,
...
void func_80A7A568(EnIn* this, PlayState* play) {
    ...
    if (gSaveContext.timerState == TIMER_STATE_STOP) {
        AudioSfx_PlaySfx(NA_SE_SY_FOUND, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultReverb);
        ...
        if (play->msgCtx.choiceIndex == 0) {
            ...
            AudioSfx_PlaySfx(NA_SE_SY_FOUND, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
