...
void DoorWarp1_SetupPurpleCrystal(DoorWarp1* this, PlayState* play) {
    ...
    if (gSaveContext.entranceIndex != ENTR_TOKINOMA_0) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_SHUT_BY_CRYSTAL);
    }
...
void DoorWarp1_WarpAppear(DoorWarp1* this, PlayState* play) {
    Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_WARP_HOLE - SFX_FLAG);
...
void DoorWarp1_ChildWarpIdle(DoorWarp1* this, PlayState* play) {
    ...
    if (DoorWarp1_PlayerInRange(this, play)) {
        ...
        AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_LINK_WARP, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
...
void func_80999FE4(DoorWarp1* this, PlayState* play) {
    if (Message_GetState(&play->msgCtx) == TEXT_STATE_NONE) {
        AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_LINK_WARP, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
