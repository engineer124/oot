...
void DoorAna_Init(Actor* thisx, PlayState* play) {
    ...
    this->actor.attentionMode = ATTENTION_MODE_0;
}
...
void DoorAna_WaitOpen(DoorAna* this, PlayState* play) {
    ...
    if (Math_StepToF(&this->actor.scale.x, 0.01f, 0.001f)) {
        if ((this->actor.attentionMode != ATTENTION_MODE_0) && (play->transitionTrigger == TRANS_TRIGGER_OFF) &&
            ...
            if (!Player_InCsMode(play) && !(player->stateFlags1 & (PLAYER_STATE1_23 | PLAYER_STATE1_27)) &&
                ...
                this->actor.attentionMode = ATTENTION_MODE_1;
            } else {
                this->actor.attentionMode = ATTENTION_MODE_0;
            }
        }
