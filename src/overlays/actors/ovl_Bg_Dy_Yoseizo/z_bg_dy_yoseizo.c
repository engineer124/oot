...
void BgDyYoseizo_ChooseType(BgDyYoseizo* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_GREAT_FAIRY_APPEAR);
...
void BgDyYoseizo_SetupSpinGrow_NoReward(BgDyYoseizo* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_VOICE_FR_LAUGH_0);
    func_8002DF54(play, &this->actor, 1);
...
void BgDyYoseizo_SetupHealPlayer_NoReward(BgDyYoseizo* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_VOICE_FR_SMILE_0);
    this->mouthState = 1;
...
void BgDyYoseizo_SetupSpinShrink(BgDyYoseizo* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_VOICE_FR_LAUGH_0);
    Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_GREAT_FAIRY_VANISH);
    ...
    for (i = 0; i < BG_DY_YOSEIZO_EFFECT_COUNT; i++, effect++) {
        if (effect->alive != 0) {
            ...
            if (effect->type == 0) {
                ...
                Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_HEALING - SFX_FLAG);
