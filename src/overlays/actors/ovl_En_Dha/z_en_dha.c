        ...
        if (Math_Vec3f_DistXYZ(&playerPos, &this->handPos[0]) <= 12.0f) {
            if (this->unk_1CC == 0) {
                if (play->grabPlayer(play, player)) {
                    ...
                    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_GRIP);
                }
            } else {
                this->timer += 0x1194;
            ...
            if (this->actor.home.rot.z != 0) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_HAND_AT);
...
void EnDha_SetupDeath(EnDha* this) {
    ...
    if (this->actor.parent != NULL) {
        if (this->actor.parent->params != ENDH_DEATH) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_HAND_DEAD);
        }
