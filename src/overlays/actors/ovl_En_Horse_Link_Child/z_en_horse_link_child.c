...
void func_80A693D0(EnHorseLinkChild* this) {
    ...
    if ((this->skin.skelAnime.curFrame > D_80A6AF5C[this->unk_1F0]) &&
        ...
        AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_KID_HORSE_WALK, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
...
void func_80A6948C(EnHorseLinkChild* this) {
    if (this->animationIdx == 2) {
        ...
        if ((this->animationIdx == 3) || (this->animationIdx == 4)) {
            AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_KID_HORSE_RUN, &this->actor.projectedPos, 4,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        } else if (this->animationIdx == 1) {
            if (Rand_ZeroOne() > 0.5f) {
                AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_KID_HORSE_GROAN, &this->actor.projectedPos, 4,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            } else {
                AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_KID_HORSE_NEIGH, &this->actor.projectedPos, 4,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            }
        }
    }
...
void func_80A6A068(EnHorseLinkChild* this, PlayState* play) {
    ...
    if (gSaveContext.entranceIndex == ENTR_SPOT20_1) {
        AudioSfx_PlaySfx(SFX_ID_ENVIRONMENT_KID_HORSE_NEIGH, &this->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        func_80A6A724(this);
        return;
    }
