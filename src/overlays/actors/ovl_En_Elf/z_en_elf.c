...
void func_80A03610(EnElf* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_FIATY_HEAL - SFX_FLAG);
}
    ...
    if ((play->csCtx.state != CS_STATE_IDLE) && (play->csCtx.npcActions[8] != NULL)) {
        ...
        if ((play->sceneId == SCENE_LINK_HOME) && (gSaveContext.sceneLayer == 4)) {
            ...
            if (play->csCtx.frames == 55) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_FAIRY_DASH);
            }
    ...
    if (this->fairyFlags & 1) {
        ...
        if ((arrowPointedActor != NULL) && (player->unk_664 != NULL)) {
            if (arrowPointedActor->category == ACTORCAT_NPC) {
                sfxId = SFX_ID_VOICE_NAVY_HELLO;
            } else {
                sfxId =
                    (arrowPointedActor->category == ACTORCAT_ENEMY) ? SFX_ID_VOICE_NAVY_ENEMY : SFX_ID_VOICE_NAVY_HEAR;
            }
                ...
                switch (this->unk_2A8) {
                    case 0:
                        if (this->unk_2C0 != 0) {
                            ...
                            if (this->unk_2C7 == 0) {
                                Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_NAVY_VANISH);
                            }
                            temp = 7;
                        }
                        break;
...
void func_80A053F0(Actor* thisx, PlayState* play) {
    ...
    if (Actor_ProcessTalkRequest(thisx, play)) {
        Audio_PlaySfx_AtPosWithReverb(&gSfxDefaultPos, SFX_ID_VOICE_SK_LAUGH, 0x20);
