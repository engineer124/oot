...
void MagicDark_DiamondUpdate(Actor* thisx, PlayState* play) {
    ...
    if (nayrusLoveTimer < 1100) {
        Actor_PlaySfx_Flagged(thisx, SFX_ID_PLAYER_MAGIC_SOUL_NORMAL - SFX_FLAG);
    } else {
        Actor_PlaySfx_Flagged(thisx, SFX_ID_PLAYER_MAGIC_SOUL_FLASH - SFX_FLAG);
    }
}
...
void MagicDark_OrbUpdate(Actor* thisx, PlayState* play) {
    ...
    Actor_PlaySfx_Flagged(&this->actor, SFX_ID_PLAYER_MAGIC_SOUL_BALL - SFX_FLAG);
    if (this->timer < 35) {
