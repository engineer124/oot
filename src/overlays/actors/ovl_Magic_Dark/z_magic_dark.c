...
void MagicDark_DiamondUpdate(Actor* thisx, PlayState* play) {
    ...
    if (nayrusLoveTimer < 1100) {
        Actor_PlaySfx_Shared(thisx, NA_SE_PL_MAGIC_SOUL_NORMAL - SFX_FLAG);
    } else {
        Actor_PlaySfx_Shared(thisx, NA_SE_PL_MAGIC_SOUL_FLASH - SFX_FLAG);
    }
}
...
void MagicDark_OrbUpdate(Actor* thisx, PlayState* play) {
    ...
    Actor_PlaySfx_Shared(&this->actor, NA_SE_PL_MAGIC_SOUL_BALL - SFX_FLAG);
    if (this->timer < 35) {
