...
void MagicDark_DiamondUpdate(Actor* thisx, PlayState* play) {
    ...
    if (nayrusLoveTimer < 1100) {
        func_8002F974(thisx, SFX_ID_PLAYER_MAGIC_SOUL_NORMAL - SFX_FLAG);
    } else {
        func_8002F974(thisx, SFX_ID_PLAYER_MAGIC_SOUL_FLASH - SFX_FLAG);
    }
}
...
void MagicDark_OrbUpdate(Actor* thisx, PlayState* play) {
    ...
    func_8002F974(&this->actor, SFX_ID_PLAYER_MAGIC_SOUL_BALL - SFX_FLAG);
    if (this->timer < 35) {
