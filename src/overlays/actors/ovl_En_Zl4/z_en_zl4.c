    ...
    switch (this->talkState) {
        case 0:
            if (this->skelAnime.curFrame == 50.0f) {
                Actor_PlaySfx(&this->actor, SFX_ID_VOICE_Z0_MEET);
            }
    ...
    switch (this->talkState) {
        ...
        case 4:
            if (this->skelAnime.curFrame == 16.0f) {
                Actor_PlaySfx(&this->actor, SFX_ID_VOICE_Z0_QUESTION);
            }
        ...
        case 7:
            if (this->skelAnime.curFrame == 17.0f) {
                Actor_PlaySfx(&this->actor, SFX_ID_VOICE_Z0_SMILE_0);
            }
    ...
    switch (this->talkState) {
        ...
        case 12:
            if (this->skelAnime.curFrame == 5.0f) {
                Actor_PlaySfx(&this->actor, SFX_ID_VOICE_Z0_SIGH_0);
            }
    ...
    switch (this->talkState) {
        ...
        case 4:
            if (!((Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) && Message_ShouldAdvance(play))) {
                ...
                this->talkState++;
                Actor_PlaySfx(&this->actor, SFX_ID_VOICE_Z0_HURRY);
            }
            break;
        case 5:
            if (EnZl4_SetNextAnim(this, ZL4_ANIM_4)) {
