...
void EnNb_PlayCrySFX(EnNb* this, PlayState* play) {
    if (play->csCtx.frames == 3) {
        Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_VOICE_NB_CRY_0);
    }
}

void EnNb_PlayAgonySFX(EnNb* this, PlayState* play) {
    if (play->csCtx.frames == 420) {
        Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_VOICE_NB_AGONY);
    }
}
...
void EnNb_PlayKnuckleDefeatSFX(EnNb* this, PlayState* play) {
    ...
    if (play->csCtx.frames == 548) {
        Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_VOICE_NB_CRY_0);
        Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_ENEMY_FANTOM_HIT_THUNDER);
    }
}
...
void EnNb_PlayKneelingOnGroundSFX(EnNb* this) {
    ...
    if ((this->skelAnime.mode == 2) &&
        ...
        Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_ENVIRONMENT_HUMAN_BOUND);
    }
}
...
void EnNb_SetNoticeSFX(EnNb* this) {
    Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_VOICE_NB_NOTICE);
}
