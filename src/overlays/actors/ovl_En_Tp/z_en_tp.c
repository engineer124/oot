...
void EnTp_Init(Actor* thisx, PlayState* play2) {
    ...
    if (this->actor.params <= TAILPASARAN_HEAD) {
        ...
        this->actor.flags |= ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_ENEMY | ACTOR_FLAG_NO_UPDATE_CULLING;
        ...
        for (i = 0; i <= 6; i++) {
            ...
            if (next != NULL) {
                ...
                if (i == 2) {
                    next->actor.flags |= ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_ENEMY | ACTOR_FLAG_NO_UPDATE_CULLING;
                    next->unk_150 = 1; // Why?
                }
...
void EnTp_Update(Actor* thisx, PlayState* play) {
    ...
    if (player->stateFlags1 & PLAYER_STATE1_TAKING_DAMAGE) { // Shielding
