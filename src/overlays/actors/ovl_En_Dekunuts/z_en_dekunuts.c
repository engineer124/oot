...
void EnDekunuts_SetupBurrow(EnDekunuts* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_NUTS_DOWN);
...
void EnDekunuts_SetupBeginRun(EnDekunuts* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_NUTS_DAMAGE);
    this->collider.base.acFlags &= ~AC_ON;
...
void EnDekunuts_SetupDie(EnDekunuts* this) {
    ...
    this->actor.speedXZ = 0.0f;
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_NUTS_DEAD);
}
...
void EnDekunuts_Wait(EnDekunuts* this, PlayState* play) {
    ...
    if (Animation_OnFrame(&this->skelAnime, 9.0f)) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_NUTS_UP);
    }
...
void EnDekunuts_Run(EnDekunuts* this, PlayState* play) {
    ...
    if (this->playWalkSfx) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_NUTS_WALK);
        this->playWalkSfx = false;
    } else {
