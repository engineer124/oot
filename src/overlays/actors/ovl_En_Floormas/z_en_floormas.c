...
void EnFloormas_SetupHover(EnFloormas* this, GlobalContext* globalCtx) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_ATTACK);
...
void EnFloormas_Die(EnFloormas* this, GlobalContext* globalCtx) {
    if (this->actor.scale.x > 0.004f) {
        ...
        Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_SPLIT);
    } else {
        // Die
...
void EnFloormas_Turn(EnFloormas* this, GlobalContext* globalCtx) {
    ...
    if (((this->skelAnime.playSpeed > 0.0f) && Animation_OnFrame(&this->skelAnime, 21.0f)) ||
        ...
        if (this->actor.scale.x > 0.004f) {
            ...
            Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_SM_WALK);
        }
    }
    // Needed to match
...
void EnFloormas_Slide(EnFloormas* this, GlobalContext* globalCtx) {
    ...
    Actor_PlaySfxAtPos3(&this->actor, NA_SE_EN_FLOORMASTER_SLIDING);
}
    ...
    if (this->actor.bgCheckFlags & 2) {
        ...
        if (this->actor.velocity.y < -4.0f) {
            if (this->actor.scale.x > 0.004f) {
                ...
                Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_SM_LAND);
            }
        }
    }
...
void EnFloormas_GrabLink(EnFloormas* this, GlobalContext* globalCtx) {
    ...
    Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_SM_STICK);
}
...
void EnFloormas_Merge(EnFloormas* this, GlobalContext* globalCtx) {
    ...
    if (((prevScale == 0.007f) || (prevScale == 0.004f)) && (prevScale != this->actor.scale.x)) {
        Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_EXPAND);
    }
    ...
    Actor_PlaySfxAtPos3(&this->actor, NA_SE_EN_FLOORMASTER_RESTORE - SFX_FLAG);
}
        ...
        if ((this->actor.colChkInfo.damageEffect != 0) || (this->actor.colChkInfo.damage != 0)) {
            if (this->collider.base.colType != COLTYPE_HARD) {
                ...
                if (Actor_ApplyDamage(&this->actor) == 0) {
                    if (isSmall) {
                        Actor_PlaySfxAtPos1(&this->actor, NA_SE_EN_FLOORMASTER_SM_DEAD);
                    } else {
