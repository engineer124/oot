...
void EnFloormas_SetupHover(EnFloormas* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_ATTACK);
...
void EnFloormas_Die(EnFloormas* this, PlayState* play) {
    if (this->actor.scale.x > 0.004f) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SPLIT);
    } else {
        // Die
...
void EnFloormas_BigWalk(EnFloormas* this, PlayState* play) {
    ...
    if (((animPastFrame || (Animation_OnFrame(&this->skelAnime, 12.0f))) ||
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_WALK);
    }
...
void EnFloormas_Turn(EnFloormas* this, PlayState* play) {
    ...
    if (((this->skelAnime.playSpeed > 0.0f) && Animation_OnFrame(&this->skelAnime, 21.0f)) ||
        ...
        if (this->actor.scale.x > 0.004f) {
            ...
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SM_WALK);
        }
    }
    // Needed to match
...
void EnFloormas_Slide(EnFloormas* this, PlayState* play) {
    ...
    Actor_PlaySfx_Flagged(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SLIDING - SFX_FLAG);
}
...
void EnFloormas_Land(EnFloormas* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
        ...
        if (this->actor.velocity.y < -4.0f) {
            if (this->actor.scale.x > 0.004f) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_LAND);
            } else {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SM_LAND);
            }
        }
    }
...
void EnFloormas_GrabLink(EnFloormas* this, PlayState* play) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SM_STICK - SFX_FLAG);
}
...
void EnFloormas_Merge(EnFloormas* this, PlayState* play) {
    ...
    if (((prevScale == 0.007f) || (prevScale == 0.004f)) && (prevScale != this->actor.scale.x)) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_EXPAND);
    }
    ...
    Actor_PlaySfx_Flagged(&this->actor, SFX_ID_ENEMY_FLOORMASTER_RESTORE - SFX_FLAG);
}
        ...
        if ((this->actor.colChkInfo.damageEffect != 0) || (this->actor.colChkInfo.damage != 0)) {
            if (this->collider.base.colType != COLTYPE_HARD) {
                ...
                if (Actor_ApplyDamage(&this->actor) == 0) {
                    if (isSmall) {
                        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FLOORMASTER_SM_DEAD);
                    } else {
                        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_DEAD);
                    }
                    Enemy_StartFinishingBlow(play, &this->actor);
                    this->actor.flags &= ~ACTOR_FLAG_0;
                } else if (this->actor.colChkInfo.damage != 0) {
                    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_FALL_DAMAGE);
                }
