    ...
    if ((this->actor.params >= ENDH_START_ATTACK_GRAB) || (this->actor.params <= ENDH_HANDS_KILLED_4)) {
        switch (this->actionState) {
            case 0:
                ...
                this->actionState++;
                this->drawDirtWave++;
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_HIDE);
                FALLTHROUGH;
            case 1:
...
void EnDh_Walk(EnDh* this, PlayState* play) {
    ...
    if (((s32)this->skelAnime.curFrame % 8) == 0) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_WALK);
    }
    if ((play->gameplayFrames & 0x5F) == 0) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_LAUGH);
    }
...
void EnDh_Attack(EnDh* this, PlayState* play) {
    ...
    switch (this->actionState) {
        case 1:
            ...
            this->actionState++;
            Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_BITE);
            FALLTHROUGH;
        case 0:
...
void EnDh_SetupDamage(EnDh* this) {
    ...
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_DEADHAND_DAMAGE);
