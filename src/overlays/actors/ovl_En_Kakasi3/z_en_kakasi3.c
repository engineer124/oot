...
void func_80A90EBC(EnKakasi3* this, PlayState* play, s32 arg) {
    ...
    switch (ocarinaNote) {
        case OCARINA_BTN_A:
            ...
            if (this->unk_1A4 == 0) {
                this->unk_1A4 = 1;
                Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_KAKASHI_ROLL);
            }
            break;
    ...
    if (this->unk_19A != 0) {
        ...
        if (this->unk_19A == 8 && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
            ...
            Actor_PlaySfx(&this->actor, SFX_ID_ITEM_KAKASHI_JUMP);
        }
        ...
        if (currentFrame == 11 || currentFrame == 17) {
            Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_KAKASHI_SWING);
        }
