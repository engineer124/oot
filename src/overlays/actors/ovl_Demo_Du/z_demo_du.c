...
void DemoDu_CsPlaySfx_GoronLanding(DemoDu* this) {
    Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_ENEMY_GOLON_LAND_BIG);
}
...
void DemoDu_CsPlaySfx_DaruniaFalling(PlayState* play) {
    if (play->csCtx.frames == 160) {
        Lib_PlaySfx_Centered(SFX_ID_ENVIRONMENT_OBJECT_FALL);
    }
}
...
void DemoDu_CsPlaySfx_DaruniaHitsLink(PlayState* play) {
    ...
    Lib_PlaySfx_AtPos(&player->actor.projectedPos, SFX_ID_ENEMY_DARUNIA_HIT_LINK);
...
void DemoDu_CsPlaySfx_HitBreast(DemoDu* this) {
    Lib_PlaySfx_AtPos(&this->actor.projectedPos, SFX_ID_ENEMY_DARUNIA_HIT_BREAST - SFX_FLAG);
}
