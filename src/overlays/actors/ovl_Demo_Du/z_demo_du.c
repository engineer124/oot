...
void DemoDu_CsPlaySfx_GoronLanding(DemoDu* this) {
    Lib_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_EN_GOLON_LAND_BIG);
}
...
void DemoDu_CsPlaySfx_DaruniaFalling(PlayState* play) {
    if (play->csCtx.curFrame == 160) {
        Lib_PlaySfx_Centered(NA_SE_EV_OBJECT_FALL);
    }
}
...
void DemoDu_CsPlaySfx_DaruniaHitsLink(PlayState* play) {
    ...
    Lib_PlaySfx_AtPos(&player->actor.projectedPos, NA_SE_EN_DARUNIA_HIT_LINK);
...
void DemoDu_CsPlaySfx_HitBreast(DemoDu* this) {
    Lib_PlaySfx_AtPos(&this->actor.projectedPos, NA_SE_EN_DARUNIA_HIT_BREAST - SFX_FLAG);
}
