...
void EnBox_Open(EnBox* this, PlayState* play) {
    ...
    if (SkelAnime_Update(&this->skelanime)) {
        ...
        if (sfxId != 0) {
            AudioSfx_PlaySfx(sfxId, &this->dyna.actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
...
void EnBox_SpawnIceSmoke(EnBox* this, PlayState* play) {
    ...
    this->iceSmokeTimer++;
    Actor_PlaySfx_Flagged(&this->dyna.actor, SFX_ID_ENEMY_MIMICK_BREATH - SFX_FLAG);
