...
static u16 sSfxIds[] = {
    SFX_ID_ITEM_ROLLING_CUT_LV2,
    SFX_ID_ITEM_ROLLING_CUT_LV1,
    SFX_ID_ITEM_ROLLING_CUT_LV2,
    SFX_ID_ITEM_ROLLING_CUT_LV1,
};
...
void EnMThunder_Init(Actor* thisx, PlayState* play2) {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_17) {
        if (!gSaveContext.isMagicAcquired || (gSaveContext.magicState != MAGIC_STATE_IDLE) ||
            ...
            AudioSfx_PlaySfx(SFX_ID_ITEM_ROLLING_CUT, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            AudioSfx_PlaySfx(SFX_ID_ITEM_SWORD_SWING_HARD, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        ...
        this->unk_1C4 = 8;
        AudioSfx_PlaySfx(SFX_ID_ITEM_ROLLING_CUT_LV1, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                         &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        this->unk_1AC = 1.0f;
    } else {
    ...
    if (player->stateFlags2 & PLAYER_STATE2_17) {
        ...
        if (player->unk_858 <= 0.15f) {
            if ((player->unk_858 >= 0.1f) && (player->meleeWeaponAnimation >= PLAYER_MWA_SPIN_ATTACK_1H)) {
                AudioSfx_PlaySfx(SFX_ID_ITEM_ROLLING_CUT, &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                                 &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
                AudioSfx_PlaySfx(SFX_ID_ITEM_SWORD_SWING_HARD, &player->actor.projectedPos, 4,
            ...
            func_80A9EFE0(this, func_80A9F9B4);
            this->unk_1C4 = 8;
            AudioSfx_PlaySfx(sSfxIds[this->unk_1C6], &player->actor.projectedPos, 4, &gSfxDefaultFreqAndVolScale,
                             &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
            this->unk_1AC = 1.0f;
            return;
        }
