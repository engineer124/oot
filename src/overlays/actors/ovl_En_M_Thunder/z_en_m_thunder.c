...
void func_80A9F408(EnMThunder* this, PlayState* play) {
    ...
    this->unk_1B8 = player->spinAttackTimer;
    this->actor.world.pos = player->bodyPartsPos[PLAYER_BODYPART_WAIST];
    this->actor.shape.rot.y = player->actor.shape.rot.y + 0x8000;
    ...
    if (player->spinAttackTimer > 0.15f) {
        ...
        this->unk_1BC += ((((player->spinAttackTimer - 0.15f) * 1.5f) - this->unk_1BC) * 0.5f);

    } else if (player->spinAttackTimer > .1f) {
        this->unk_1C8 = (s32)((player->spinAttackTimer - .1f) * 255.0f * 20.0f);
        this->unk_1AC = (player->spinAttackTimer - .1f) * 10.0f;
    } else {
        this->unk_1C8 = 0;
    }
