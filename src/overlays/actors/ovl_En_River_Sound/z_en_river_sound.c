...
void EnRiverSound_Draw(Actor* thisx, PlayState* play) {
    static s16 soundEffects[] = {
        ...
        SFX_ID_ENVIRONMENT_CROWD - SFX_FLAG,
        0,
        SFX_ID_ENVIRONMENT_SARIA_MELODY - SFX_FLAG,
        0,
        SFX_ID_ENVIRONMENT_SAND_STORM - SFX_FLAG,
        SFX_ID_ENVIRONMENT_WATER_BUBBLE - SFX_FLAG,
        SFX_ID_ENVIRONMENT_KENJA_ENVIROMENT_0 - SFX_FLAG,
        SFX_ID_ENVIRONMENT_KENJA_ENVIROMENT_1 - SFX_FLAG,
        SFX_ID_ENVIRONMENT_EARTHQUAKE - SFX_FLAG,
        0,
        SFX_ID_ENVIRONMENT_TORCH - SFX_FLAG,
        SFX_ID_ENVIRONMENT_COW_CRY_LV - SFX_FLAG,
    };
    ...
    if (!this->playSfx) {
        ...
        Audio_UpdateRiverSoundMainBgm(&this->actor.projectedPos, this->actor.xzDistToPlayer);
    } else if (this->actor.params == RS_GORON_CITY_SARIAS_SONG) {
        // Play Sarias Song in Goron City at the entrance to lost woods
        // Volume depends on distance to source
        Audio_UpdateRiverSoundSubBgm(&this->actor.home.pos, SEQ_ID_SARIA_THEME, 1000);
    } else if (this->actor.params == RS_GREAT_FAIRY) {
        // Play the Great Fairy Song inside the fairy fountain
        // Volume depends on distance to source
        Audio_UpdateRiverSoundSubBgm(&this->actor.home.pos, SEQ_ID_GREAT_FAIRY, 800);
        ...
        Lib_PlaySfx_Centered(soundEffects[this->actor.params]);
    } else {
