...
#define FLAGS (ACTOR_FLAG_NO_UPDATE_CULLING | ACTOR_FLAG_HOOK_BRING_PLAYER)
            ...
            if (this->litTimer != 0) {
                ...
                if ((0 <= this->litTimer) && (this->litTimer < (50 * litTimeScale + 100)) && (torchType != 0)) {
                    this->litTimer = 50 * litTimeScale + 100;
                                            ...
                                            ((interactionType < 0) && (player->stickFlameTimer != 0)))) {

                if ((interactionType < 0) && (player->stickFlameTimer < 200)) {
                    player->stickFlameTimer = 200;
                }
                if (torchCount == 0) {
                    this->litTimer = -1;
