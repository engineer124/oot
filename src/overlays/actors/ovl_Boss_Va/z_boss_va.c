...
void BossVa_BodyIntro(BossVa* this, PlayState* play) {
    ...
    switch (sCsState) {
        ...
        case INTRO_DOOR_SHUT:
            ...
            if (Rand_ZeroOne() < 0.1f) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_BL_SPARK - SFX_FLAG);
            }
            break;
        case INTRO_CRACKLE:
            func_8002DF54(play, &this->actor, 1);
        ...
        case INTRO_ATTACH_BARI:
            for (i = 10; i >= 1; i--) {
                if (sBodyBari[i - 1]) {
                    if (sBodyBari[i - 1] == 1) {
                        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_STICK);
    ...
    if (sCsState >= INTRO_BODY_SOUND) {
        Audio_PlaySfx_AtPosWithFreq(&this->actor.projectedPos, SFX_ID_ENEMY_BALINADE_LEVEL - SFX_FLAG, 1.0f);
...
void BossVa_BodyPhase1(BossVa* this, PlayState* play) {
    ...
    if (sBodyState & 0x7F) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_DAMAGE);
    }
...
void BossVa_BodyPhase2(BossVa* this, PlayState* play) {
    ...
    if (this->colliderBody.base.acFlags & AC_HIT) {
        ...
        Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_FAINT);
    }
    ...
    Audio_PlaySfx_AtPosWithFreq(&this->actor.projectedPos, SFX_ID_ENEMY_BALINADE_LEVEL - SFX_FLAG,
...
void BossVa_BodyPhase4(BossVa* this, PlayState* play) {
    ...
    if (this->colliderBody.base.acFlags & AC_HIT) {
        ...
        if (this->timer >= 0) {
            ...
            Actor_PlaySfx(&this->actor, SFX_ID_ITEM_SHIELD_REFLECT_SW);
        }
    ...
    switch (sCsState) {
        ...
        case DEATH_CORE_DEAD:
            ...
            if (!this->isDead) {
                ...
                this->timer = 30;
                sCsState++;
                EffectSsDeadSound_SpawnStationary(play, &this->actor.projectedPos, SFX_ID_ENEMY_BALINADE_DEAD, 1, 1,
                                                  0x28);
                this->onCeiling = 2; // Not used by body
        ...
        case DEATH_CORE_BURST:
            if (!this->burst) {
                ...
                if (this->timer2 >= 32) {
                    this->burst++;
                    this->isDead = true;
                    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_BREAK2);
...
void BossVa_ZapperAttack(BossVa* this, PlayState* play) {
    ...
    if ((sp8E >= ABS(tmp17) || this->burst) && !(sBodyState & 0x80) && !(player->stateFlags1 & PLAYER_STATE1_26)) {
        ...
        if (this->burst || (this->timer2 < 0)) {
            if (this->colliderLightning.base.atFlags & AT_HIT) {
                if (this->timer2 > 0) {
                    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_HIT_RINK);
                    BossVa_SetSparkEnv(play);
                    this->timer2 = -1;
    ...
    if (this->burst && (this->burst != 2)) { // burst can never be 2
        if (this->timer2 >= 32) {
            if (this->timer2 == 32) {
                Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_THUNDER);
            }
...
void BossVa_SetupBariDeath(BossVa* this) {
    this->actor.flags &= ~ACTOR_FLAG_0;
    this->timer = 30;
    Actor_PlaySfx(&this->actor, SFX_ID_ENEMY_BALINADE_BL_DEAD);
    this->isDead++;
...
void BossVa_Door(BossVa* this, PlayState* play) {
    if (sDoorState == 29) {
        Actor_PlaySfx(&this->actor, SFX_ID_ENVIRONMENT_BUYODOOR_CLOSE);
    }
...
void BossVa_SpawnTumor(PlayState* play, BossVaEffect* effect, BossVa* this, Vec3f* offset, s16 scale, u8 mode) {
    ...
    for (i = 0; i < BOSS_VA_EFFECT_COUNT; i++, effect++) {
        if (effect->type == VA_NONE) {
            ...
            if (((i % 4) == 0) || (mode == 2)) {
                AudioSfx_PlaySfx(SFX_ID_ENEMY_BALINADE_BREAK, &effect->pos, 4, &gSfxDefaultFreqAndVolScale,
