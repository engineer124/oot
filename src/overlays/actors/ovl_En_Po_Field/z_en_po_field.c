...
void EnPoField_SetupWaitForSpawn(EnPoField* this, PlayState* play) {
    ...
    this->actor.flags &= ~(ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_IMMEDIATE_TALK);
...
void EnPoField_SetFleeSpeed(EnPoField* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    f32 speed = ((player->stateFlags1 & PLAYER_STATE1_RIDING_HORSE) && player->rideActor != NULL)
                    ? player->rideActor->speed
                    : 12.0f;
        ...
        for (i = 0; i < sNumSpawned; i++) {
            if (fabsf(sSpawnPositions[i].x - player->actor.world.pos.x) < 150.0f &&
                ...
                if (Flags_GetSwitch(play, sSpawnSwitchFlags[i])) {
                    if (player->stateFlags1 & PLAYER_STATE1_RIDING_HORSE) { // Player riding Epona
                        return;
                    } else {
                        this->actor.params = EN_PO_FIELD_SMALL;
                        spawnDist = 300.0f;
                    }
                } else if (player->stateFlags1 & PLAYER_STATE1_RIDING_HORSE || Rand_ZeroOne() < 0.4f) {
